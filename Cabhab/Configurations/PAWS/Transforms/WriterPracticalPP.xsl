...
<xsl:template match="/" mode="pp">
    <section1 id="sPP">
        ...
        <p>
<xsl:text>This section covers the internal structure of prepositional or postpositional phrases.  Prepositions come before their complements, while postpositions come afterward.  English has prepositions, as shown in </xsl:text>
<langData>
...
<xsl:text>. Ordinal numbers and adjectives can also modify </xsl:text>
<xsl:choose>
                    <xsl:when test="//pp/@pPos='before'">prepositional</xsl:when>
                    <xsl:when test="//pp/@pPos='after'">postpositional</xsl:when>
                    <xsl:when test="//pp/@pPos='both'">prepositional or postpositional</xsl:when>
...
<xsl:text></xsl:text>
         ...
         <section2 id="sPPComps">
             <secTitle>
<xsl:choose>
                 <xsl:when test="//pp/@pPos='before'">Complements to Prepositional Phrases</xsl:when>
                 <xsl:when test="//pp/@pPos='after'">Complements to Postpositional Phrases</xsl:when>
                 <xsl:when test="//pp/@pPos='both'">Complements to Prepositional and Postpositional Phrases</xsl:when>
                 <xsl:when test="//pp/@pPos='unknown'">Complements to Prepositional and/or Postpositional Phrases</xsl:when>
             </xsl:choose>
</secTitle>
            <p>
<xsl:choose>
...
<xsl:text>  can stand alone without a complement, such as in the sentences </xsl:text>
<langData>
<xsl:attribute name="lang">
<xsl:text>lPAWSSKEnglish</xsl:text>
...
<xsl:text> to be stranded in questions, as in </xsl:text>
<langData>
<xsl:attribute name="lang">
<xsl:text>lPAWSSKEnglish</xsl:text>
