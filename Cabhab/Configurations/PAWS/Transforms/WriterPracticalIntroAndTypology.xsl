...
<xsl:text>.  This descriptive grammar is written as a practical grammar that should be useful for the language community as well as for documenting much of the language.</xsl:text>
</p>
		<p>
...
<xsl:text>.  Basic, main clause sentences are then covered in section </xsl:text>
<sectionRef sec="sIP" />
<xsl:text>, followed by constructions involving changes in word order (sections </xsl:text>
<sectionRef sec="sFocus" />
<xsl:text> - </xsl:text>
<sectionRef sec="sNeg" />
<xsl:text>).  Coordination constructions are then covered in section </xsl:text>
<sectionRef sec="sCoord" />
<xsl:text>, followed finally by various types of embedded clauses (sections </xsl:text>
