...
*-----------------------------------------------------------*/
#ifdef DJGPP
#define VERSION "Phonological Rule 0.2w  Dec 2000 Copyright SIL International\n"\
...
if ( !rulefile )        /* Require rule file */
	{
	fprintf( stderr, "Error: Rule file -r required\n" );
	++errflag;
	}

if ( !infile )          /* Require input file */
	{
	fprintf( stderr, "Error: Input file -i required\n" );
	++errflag;
	}
