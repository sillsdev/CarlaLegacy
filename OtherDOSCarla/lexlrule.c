 ...
 *      loadrules
 * ARGS
 *      dicfile - single dictionary file to load (from command line)
 *      cmdfile - AMPLE command file to get dictionary files from
 * DESCR
...
if ( cmdfile )                              /* 0.2k BJY */
	if ( !read_codetab( cmdfile ) )         /* look for noload marker(s) */
		return( NULL );                     /* error in command file or code table */
...
if ( do_debug )
	{
	printf( "Rec marker is: \"%s\", length: %d\n", recmark, rmarklen );
	printf( "Src marker is: \"%s\", length: %d\n", sourcemark, smarklen );
	printf( "Tar marker is: \"%s\", length: %d\n", targetmark, tmarklen );
	printf( "Cat marker is: \"%s\", length: %d\n", catmark, cmarklen );
	printf( "Prop marker is: \"%s\", length: %d\n", propmark, pmarklen );
	}
...
if ( dicfile )                          /* 0.2k BJY filename from command line */
	infile = dicfile;
else
	infile = next_rootfile( cmdfile );  /* dictionary filenames coming from command file */
...
do  {
	...
	} while( !dicfile && ( infile = next_rootfile( cmdfile ) ) );  /* 0.2k BJY */
...
if ( !hash[ *from ] )               /* If hash entry of letter is null */
	...
	if ( strcmp( t->match, h->match ) < 0 ) /* If lower than first */
	  ...
	  {
			printf( "Skipping duplicate rule %s -> %s\n",
					from, t->repl );
