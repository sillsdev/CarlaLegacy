		...
		if ( !streq( s, "->" ) && !streq( s, ">" ) )
			...
			if ( *ru->cat == '[' )      /* If category class */
				...
				if ( !t || *( t + 1 ) ) /* If no ] or ] not last */
					{
					printf( "Error: Invalid category class\n%s", line );
					goto badrule;
					}
				else
					{
					*t = '\0';          /* Terminate class */
					if ( !find_ccl( ru->cat + 1 ) )
						{
						printf( "Error: Undefined category class\n%s", line );
						goto badrule;
						}
					}
				}
				...
				if ( !findcat( ru->cat ) )
					{
					printf( "Error: Undefined category %s\n%s", ru->cat, line );
					goto badrule;
					}
				}       /* end 0.2n BJY */
			...
			if ( !streq( s, "->" ) && !streq( s, ">" ) )
				{
				printf( "Error: Arrow not found in rule\n%s", line );
				goto badrule;
				}
			}
...
#ifndef hab02w
	else if ( !strncmp( "\\CSbegca ", wkline, 9 ) )
		{
		valid_mkr = TRUE;       /* ignore \CSbegca field */
		}
	else if ( !strncmp( "\\CSbegccl ", wkline, 10 ) )
		{
		valid_mkr = TRUE;       /* ignore \CSbegccl field */
		}
	else if ( !strncmp( "\\CSend ", wkline, 7 ) )
		{
		valid_mkr = TRUE;       /* ignore \CSend field */
		}
	else if ( !strncmp( "\\CSbegru ", wkline, 9 ) )
		{
		valid_mkr = TRUE;       /* ignore \CSbegru field */
		}
#endif /* hab02w */
	else    /* 0.2o BJY  Check for undefined marker */
		{
		if ( *wkline == '\\' && !valid_mkr )
			printf( "Warning: Undefined field\n%s", line );
		}
...
while (TRUE)
	...
	if ( !m )       /* no more classes on match side */
		...
		if ( r )    /* more classes in replacement than match */
			{
			printf( "Error: Unmatched replacement class in rule\n%s", line );
			return( FALSE );
			}
		else
	...
	*endcl = ']';                   /* Restore class terminator */
	...
	if ( mstr || rstr )
		{
		printf( "Error: Mismatched classes in rule\n%s", line );
		return( FALSE );
		}
	}
}
...
for ( ; *s; s++ )
	...
	if ( *s == '[' )                    /* If class */
		...
		if ( !endcl )                   /* If no end, fail */
			{
			printf( "Error: Unterminated class %s in rule\n%s", s, line );
			return( FALSE );
			}
		*endcl = '\0';                  /* Terminate class */
		...
		if ( !scl )                     /* If class not found, fail */
			{
			printf( "Error: Undefined class [%s] in rule\n%s", s+1, line );
			*endcl = ']';                   /* Restore class terminator */
			return( FALSE );
			}
		*endcl = ']';                   /* Restore class terminator */
	...
	if ( *s == '[' && *(s+1) )      /* If class reference */
		...
		if ( *t == ']' )                        /* If close bracket, delete it */
			*t = '\0';
			...
			printf( "Error: Missing ] on string class %s\n%s", s, line );
		tscl =  find_scl( s );
