...
CModelFilesSet::CModelFilesSet(LPCTSTR lpszAbrev)
:m_sAbrev(lpszAbrev),
m_bUseUnified(TRUE),
	m_pPreprocessedDicts(NULL)
...
*/
	...
	if(m_sAdctlFileDirectory.GetLength())
		sout << "\\DIRadctl " << T2CA(m_sAdctlFileDirectory) << "\n";
	if(m_sAdctlFileName.GetLength())
		sout << "\\adctl " << T2CA(m_sAdctlFileName) << "\n";

	// note that unified dicts names are found in this m_sRootDictNames
	if(!m_rootDicts.GetSize())	// added to try to find Terry's bug.
	{
		CString s;
		s.Format(_T("Warning: there are no root or unified dictionaries currently listed for language %s."), (LPCTSTR)m_sAbrev);
		AfxMessageBox(s);
	}
	for( int i=0; i< m_rootDicts.GetSize(); i++)
	{
		...
		{
			CString sMsg;
			sMsg.Format(_T("CarlaStudio has lost the path leading up to the dictionary %s.  To fix this, go to the Language menu for %s, choose Properties, and Choose the 'Dicts' tab.  Please report this as a bug."), (LPCTSTR)s, (LPCTSTR)m_sAbrev);
			AfxMessageBox(sMsg);
		}
	}
...
BOOL CModelFilesSet::recognizeField(CString sMarker, CString sField)
{
	if(sMarker.Left(4) == _T("DICT"))
	{
		...
		if(sField.GetLength()>0 && -1 == sField.FindOneOf(_T("/\\")))
		{
	...
	else if(sMarker == _T("DICTpfx"))
	{
		m_pathPrefixDict= sField;
		m_bUseUnified = FALSE;
	}
	else if(sMarker == _T("DICTifx"))
	{
		m_pathInfixDict = sField;
		m_bUseUnified = FALSE;
	}
	else if(sMarker == _T("DICTsfx"))
	{
		m_pathSuffixDict = sField;
		m_bUseUnified = FALSE;
	}
	else if(sMarker == _T("DICTRootOrUnified"))
	{
		if(sField.IsEmpty())
		{
			CString s;
			s.Format(_T("Report this error: when reading DictRootOrUnified field, the path to the dict was empty. Go to the Language menu for language %s, choose the Properties item, and check that all your dictionaries are there."), (LPCTSTR)m_sAbrev);
			AfxMessageBox(s);
		}
		else
		{
		...
		{
			CString s;
			s.Format(_T("CarlaStudio has lost the path leading up to the dictionary %s.  To fix this, go to the Language menu for %s, choose Properties, and Choose the 'Dicts' tab.  Please report this as a bug."), (LPCTSTR)sField, (LPCTSTR)m_sAbrev);
			AfxMessageBox(s);
		}
	...
	else if(sMarker == _T("OutputDirAnalyzedANA"))
	{
		m_pathDirAnalyzedANA = sField;
	}
	else if(sMarker == _T("OutputDirInterlinear"))
	{
		m_pathDirInterlinear = sField;
	}
	else if(sMarker == _T("OutputDirSynthesizedTexts"))
	{
		m_pathDirSynthesized = sField;
	}
	else if(sMarker == _T("OutputDirTransferedANA"))
	{
...
{
	ASSERTX(m_sAbrev.GetLength());
	return m_sCtrlFilesDirectory + m_sAbrev + _T("phon.rul");
}

CString CModelFilesSet::getIntxFileFullName() const
{
	ASSERTX(m_sAbrev.GetLength());
	return m_sAbrev + _T("intx.ctl");
}
...
{
	return getCtrlFilePath(_T("synt.chg"));
}
...
{
	if( _taccess(lpszPath, 0))// ask for existence only
	{
		ASSERTX(FALSE);
...
void CModelFilesSet::copyFileToCtrlDir(CString & sFromPath)
{
	...
	{	CString s(_T("Couldn't copy the file:"));
		s += sFromPath;
		s += _T(" to ");
		s += sToPath;
...
void CModelFilesSet::setOutputDirSynthesizedText(CPathDescriptor *pDir)
{
	if(pDir)
		...
		m_pathDirSynthesized = _T(""); // use default
...
void CModelFilesSet::setOutputDirAnalyzedANA(CPathDescriptor *pDir)
{
	if(pDir)
		...
		m_pathDirAnalyzedANA = _T(""); // use default
...
void CModelFilesSet::setOutputDirTransferedANA(CPathDescriptor *pDir)
{
	if(pDir)
		...
		m_pathDirTransferedANA = _T(""); // use default
...
void CModelFilesSet::setOutputDirInterlinear(CPathDescriptor *pDir)
{
	if(pDir)
		...
		m_pathDirInterlinear = _T(""); // use default
...
void CModelFilesSet::setDefaultInputDir(CPathDescriptor* pDir)
{
	if(pDir)
		...
		m_pathDirDefaultInput = _T(""); // use default
}
...
CPathDescriptor CModelFilesSet::getOutputDirInterlinear(BOOL bDefaultIfEmpty/*=TRUE*/) const
{
	...
	else
	{
		...
		s+=getAbrev() + _T(" Interlinear Files\\");
		ensureDirExists(s);
		return  (s);
	}
}
...
CPathDescriptor CModelFilesSet::getOutputDirSynthesizedText(BOOL bDefaultIfEmpty/*=TRUE*/) const
{
	...
	{
		CString s = getParentDirectory(m_sCtrlFilesDirectory);
		s+=getAbrev() + _T(" Synthesized Files\\");
		ensureDirExists(s);
		return  (s);
	}
...
CPathDescriptor CModelFilesSet::getOutputDirTransferedANA(BOOL bDefaultIfEmpty/*=TRUE*/) const
{
	...
	{
		CString s = getParentDirectory(m_sCtrlFilesDirectory);
		s+=getAbrev() + _T(" Transfered Files\\");
		ensureDirExists(s);
		return  (s);
	}
}
...
void CModelFilesSet::copyInPreprocessedDictArray(CPathDescArray* pDicts)
{
	...
	{
		CString s;
		s.Format(_T("copyIn: %s\n"), (LPCTSTR)((*pDicts)[i])->getFullPath());
		TRACE(s);
