...
void CEnvironmentListCtrl::populate()
{
	...
	m_pBlankEntry = new CStringEnvironment(_T(""), _T(""), m_pRule->m_iColumnForEnv, m_pRule);
...
void CEnvironmentListCtrl::deleteItem(LPARAM pItem, int nItem)
{
	...
	{
		AfxMessageBox(_T("This environment is a blank that you can fill in.  You cannot delete it."));
		return;
	}
