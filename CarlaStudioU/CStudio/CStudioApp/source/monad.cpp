...
{
	AfxMessageBox(_T("Edit this item by double-clicking on the label or the comment itself"), MB_OK|MB_ICONINFORMATION);
...
void CWMonad::write(ostream &fout, char cCommentChar) const
{
	...
	USES_CONVERSION_U8;
	fout << "\\" << T2CU8(((CWMonadList*)m_pOwningList)->getSFMCode()) << " ";
...
{
	insertColumn(clc, _T("Comment"),	500, 0);
	insertColumn(clc, _T("Label"),200, 0);
}
