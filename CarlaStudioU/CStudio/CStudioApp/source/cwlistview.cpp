...
{
#ifndef hab241
		pCmdUI->SetText(LPCTSTR(CString(_T("&Insert ")) + m_pList->getDisplayName()) + CString(_T("\tCtrl-I")));
#else // hab241
		pCmdUI->SetText(LPCTSTR(CString(_T("&Insert ")) + m_pList->getDisplayName()));
#endif // hab241
}
...
#else // hab17a1
		pCmdUI->SetText(LPCTSTR(CString(_T("Delete ")) + pItem->getDisplayName()));
#endif //hab17a1
	}
}
...
void CWListView::OnUpdateEditInsertSecondary(CCmdUI* pCmdUI)
{
	if(m_pList->haveSecondaryEntryType())
	{
		pCmdUI->Enable(TRUE);
		CString s;
		s.Format(_T("I&nsert %s..."), (LPCTSTR)m_pList->getSecondaryEntryTypeDisplayName());
		pCmdUI->SetText(s);
	}
	else
		pCmdUI->Enable(FALSE);
...
//end test
	ASSERTX(m_pimageListDrag == NULL);
TRACE(_T("OnBegindrag: CreateDragImage..."));
	m_pimageListDrag = m_listCtrl.CreateDragImage(m_iItemDrag, &ptImage);
TRACE(_T("done\n"));
	m_sizeDelta = ptAction - ptImage;   // difference between cursor pos and image pos
	m_ptHotSpot = ptAction - ptItem + m_ptOrigin;  // calculate hotspot for the cursor
...
LONG CWListView::doFindReplace(FINDREPLACE * pFR)
{
	...
	CString s;
	s.Format(_T("Cannot find string \"%s\""), pFR->lpstrFindWhat);
	AfxMessageBox(s);
	return 0;
}
...
void CWListView::OnEditPaste()
{
	...
	//if(m_iClipboardOperation == ID_EDIT_COPY || m_iClipboardOperation == ID_EDIT_CUT)
	{
		if(m_pClipboardRowItem->IsKindOf(RUNTIME_CLASS(CWListEntry)))
		{
			...
			if(!pNewEntry)
			{
				AfxMessageBox(_T("Sorry, pasting hasn't been implemented for this type yet."));
				return;
			}
		...
		else if(m_pClipboardRowItem->IsKindOf(RUNTIME_CLASS(CStringEnvironment)))
		{
			...
			if(!pEnv)
			{
				AfxMessageBox(_T("There was an error while trying to paste that item."));
				return;
			}
			...
			AfxMessageBox(_T("Sorry, pasting that kind of item hasn't been implemented yet."));
...
//	else
//		AfxMessageBox(_T("Sorry, that function is still under construction."));
}
...
void CWListView::OnEditCopy()
{
	...
	if(!m_pClipboardRowItem)
		AfxMessageBox(_T("Sorry, copying that kind of item isn't implemented yet."));
}
...
void CWListView::OnUpdateEditUndo(CCmdUI* pCmdUI)
{
	...
	{
		case ID_EDIT_CUT:
			pCmdUI->SetText(_T("Undo Cut"));
			break;
		case ID_Edit_Delete:
			pCmdUI->SetText(_T("Undo Delete"));
			break;
		default: break;
	}
}
