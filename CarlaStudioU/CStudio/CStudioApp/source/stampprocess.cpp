...
void CStampProcess::readParametersFromSFMFile(SFMFile *f)
{
	...
	while(f->getField(sMarker, sField, &bEnabled))
	{
		...
		{
			f->throwParseFailure(_T("StampProcess"), sMarker, sField);
		}
	}
}
...
LPCTSTR CStampProcess::getToolPath()
{
	return _T("STAMP32");
}
...
CString CStampProcess::getCommandLine(CProcessStatus& status)
{
	...
	USES_CONVERSION_U8;
	sout << "-i " << T2CA(INPUTFILE.getQuotedPath());
	sout << " -f " << T2CA(m_sCMDPath.getQuotedPath());
...
*/
	...
	fout << T2CA(getTransferPath(status)) << "\n";
	fout << T2CA(getSynthPath(status))<< "\n";
#ifndef hab17a1
...
{
	LPCTSTR kOutputExt = _T(".txs");
	LPCTSTR kLogExt = _T(".trp");

	try
	{
		ASSERTX(status.getOutputLang());
		CString sBase = status.getInputMFS()->getAbrev() + status.getOutputMFS()->getAbrev() + _T("-trample");
		...
		CResultStreamFile* textStream =
			new CResultStreamFile(
					new CResultStreamDescriptor(this,
												_T("TRMP-Target ANA"),
												_T("Target ANA produced by applying Trample (STAMP transfer) transfer rules"),
...
void CSampleProcess::processANAFile(CProcessStatus& status)
{
	...
	LPCTSTR kLogExt = _T(".smp");
	LPCTSTR kCMDExt = _T(".cms");

	try
	{
		ASSERTX(status.getOutputLang());
		CString sBase = status.getInputMFS()->getAbrev() + status.getOutputMFS()->getAbrev() + _T("-sample");
		m_sOutPath = status.makeTempPath(INPUTFILE.getFileName(), kOutputExt);
		m_sLOGPath = status.makeTempPath(sBase,	_T(".log") );
		m_sCMDPath =status.makeTempPath(sBase, _T(".cmd"));
		...
		CResultStreamFile* textStream =
			new CResultStreamFile(
					new CResultStreamDescriptor(this,
												_T("Synthesized Text"),
												_T("Target text synthesized by Sample (STAMP synthesis)"),
												sLabel // language specific
											),
					m_sOutPath.getFullPath(),
				status.getOutputLang());
...
CString CTrampleProcess::getBatchName(CProcessStatus& status)
{
	...
	CString sBatchFile;
#ifndef hab204
	sBatchFile.Format(_T("%s-%s-trample%d"),
...
CString CSampleProcess::getBatchName(CProcessStatus& status)
{
	...
	CString sBatchFile;
#ifndef hab204
	sBatchFile.Format(_T("%s-sample%d"),
			  (LPCTSTR)status.getOutputMFS()->getAbrev(),
			  status.m_iProcNumber);
#else // hab204
