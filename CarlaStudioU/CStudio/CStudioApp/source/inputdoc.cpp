...
// rde270   25-Jun-2002 Add some member functions to stop loading the files in the rich edit
//                      controls if the user starts another processing cycle
// rde269   15-Jul-2002 Don't send Shoebox the "Refresh All" command if we're processing
//                      a remote command *from* shoebox (it'll do it on its own).
#include "stdafx.h"
...
void CInputDoc::OnRunTrace()
{
	...
	{
		if (IDOK != AfxMessageBox(_T("Normally, you want to limit your traces to a single word.  Click OK to trace anyways."), MB_OKCANCEL))
			return;
	}

	if(m_pBaseStream->getFileSize() > kMaxBeforeTraceUnwise)
	{
		if (IDOK != AfxMessageBox(_T("No, really.  The trace from a file this large could fill up your hard drive.  Click OK to trace at your own risk."), MB_OKCANCEL))
			return;
	}
...
{
	CPropertySheet dlg(_T("Processing Goal"));
...
/*	kSourceAna,		// analysis and auto disambiguation
	...
	case CProcessingPrefs::kSourceAna:
		...
		case CProcessingPrefs::kTargetText:
			...
			s+=_T(")");
			break;
		default: _T(" Error: unknown goal"); break;
	}
	if(IsModified())
		s+=_T("*");
...
BOOL CInputDoc::doRunProcessors(CRemoteCommand* pRemoteCmd/*=NULL*/)
{
  ...
  for (pos = pT->GetFirstDocPosition();
	...
	if( getCanDoAnalysis())
	{
		...
		if(bOK )
		{
			CPathDescriptor newPath(pSourceLang->getMFS()->getOutputDirAnalyzedANA()+m_pProcessStatus->m_sFileNameRoot+_T(".ana"));
	...
	if(bOK && getProcessingPrefs()->getDoInterlinearize())
	{
		...
		if(bOK )//&& GOAL==CProcessingPrefs::kGlossedInterlinear)
		{
			...
			if(pRemoteCmd)
			{
				switch(pRemoteCmd->eOutputLocation)
				{
					...
					case CRemoteCommand::csReplaceInput:
							...
							break;
					default: throw(_T("Unknown OutputLocation setting"));
							break;
				}
			}
			...
			if(newPath.GetLength() == 0)
				newPath = sDestDir+m_pProcessStatus->m_sFileNameRoot+_T(".itx");
	...
	if(bOK && getProcessingPrefs()->getDoTransfer())
	{
		...
		if(bOK )//&& GOAL==CProcessingPrefs::kTargetANA)
		{
			...
			//CString z = _T("s") + x  + _T("v");

			CPathDescriptor newPath(pTargetLang->getMFS()->getOutputDirTransferedANA()+m_pProcessStatus->m_sFileNameRoot+_T(".ana"));
			redirectOutputFile(GOAL, CProcessingPrefs::kTargetANA,
				m_pProcessStatus->sANAPath,
				newPath,
				m_pProcessStatus);
			//m_pProcessStatus->sANAPath.copyTo(pTargetLang->getMFS()->getOutputDirTransferedANA()+m_pProcessStatus->m_sFileNameRoot+_T(".ana"));
		...
		if(bOK)// && GOAL==CProcessingPrefs::kTargetText)
		{
			...
			if(pRemoteCmd)
			{
				switch(pRemoteCmd->eOutputLocation)
				{
					...
					case CRemoteCommand::csReplaceInput:
							throw(_T("Whoops.  You probably didn't mean to say that CStudio should do a transfer that overwrites the initial file."));
							break;
					default: throw(_T("Unknown OutputLocation setting"));
							break;
				}
			...
			if(newPath.GetLength() == 0)
				newPath = sDestDir+m_pProcessStatus->m_sFileNameRoot+_T("_")+pTargetLang->getAbrev()+_T(".txt");
...
#pragma message("the next line was indented as if it were part of the preceding 'if', but it's not (i.e. no brackets). Should it be?")
...
CResultStream* CSourceTextInputDoc::getBaseStream()
{
	if(!m_pBaseStream)
	{
		...
		new CResultStreamDescriptor(NULL,
								_T("Src File"),
								_T("Source Text File") // long description
								);
...
void CSourceTextInputDoc::readParams(SFMFile & f)
{
	...
	while(f.getField(sMarker, sField))
	{
		...
		else THROWSTRING2(_T("CSourceTextInputDoc doesn't understand the marker: "), sMarker);
	}
}
...
void CAnaInputDoc ::readParams(SFMFile & f)
{
	...
	while(f.getField(sMarker, sField))
	{
		...
		else THROWSTRING2(_T("CAnaInputDoc  doesn't understand the marker: "), sMarker);
	}
}
