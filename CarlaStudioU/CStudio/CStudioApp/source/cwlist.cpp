...
void CWList::insertColumn(CListCtrl &clc, LPTSTR pszName, int iWidth, int nIconIndex/*=-1*/)
{	LV_COLUMN lv;
...
void CWTestList::insertColumns(CListCtrl &clc)
{
	...
	insertColumn(clc, _T("      Label"),200);
	//insertColumn(clc, _T(""), 30);	// enabled/disabled
}
 ...
 CString CWTest::getText(int iColumn)
{
	 switch(iColumn)
	{
		//case 0: return m_bEnabled?CString(_T("abcd")):CString(_T("OFF"));
		//		break;
...
{
	insertColumn(clc, _T("Example"),	300);
	insertColumn(clc, _T("Morph 2 Type"), 80);
	insertColumn(clc, _T("Morpheme 2"), 100);
	insertColumn(clc, _T("     Morpheme 1"), 120);
	//insertColumn(clc, _T(""), 30);	// enabled/disabled
}
 ...
 CString CWAdhocPair::getText(int iColumn)
{
	...
	//			break;
		...
		case 2: switch(m_cTypeOfSecondMorpheme)
				{
					case 'p': s = _T("prefix"); break;
					case 'i': s = _T("infix"); break;
					case 's': s = _T("suffix"); break;
					case 'r': s = _T("root"); break;
					case 'n': s = _T("interfix"); break;
				}
				return s;
				break;
...
{
	return _T("Compound Root Pair");
}

void CWCompoundRootPairList::insertColumns(CListCtrl &clc)
{
	insertColumn(clc, _T("Example"),	300);
	insertColumn(clc, _T("Category 2"), 100);
	insertColumn(clc, _T("      Category 1"), 120);
	//insertColumn(clc, _T(""), 30);	// enabled/disabled
}
...
void CWListRowItem::setText(int iColumn, LPCTSTR lpszStr)
{
	...
	AfxMessageBox(_T("Whoops.  You have to double click on this item's icon to edit it.  Please report this msg as a bug."));
}

 const CString CWList::getDisplayName() const
{
	return _T("?????");	// must override
}
...
BOOL CWAdhocPair::doEditDialog(CListCtrl& clc, BOOL bIsNew)
{
	...
	{
		case 'p': dlg.m_sMorphType = _T("Prefix");
			break;
		case 'i': dlg.m_sMorphType = _T("Infix");
			break;
		case 'n': dlg.m_sMorphType = _T("Interfix");
			break;
		case 'r': dlg.m_sMorphType = _T("Root");
			break;
		case 's': dlg.m_sMorphType = _T("Suffix");
			break;
		case 0: dlg.m_sMorphType = _T("Prefix");
			break;
	}
...
#ifndef hab211
void CWList::write(ostream & fout, char cCommentChar, LPTSTR pszCode) const
{
	USES_CONVERSION_U8;
	int iSize = m_pEntries.GetSize();
	if (iSize)
	  fout << "\\CSbeg" << T2CU8(pszCode) << "\n";	// indicate begin type
...
void CWAdhocPair::write(ostream& fout, char cCommentChar) const
{
	switch(m_cTypeOfSecondMorpheme)
	{
		...
		default: TRACE(_T("bad adhoc pair type\n"));
				fout << "\\pah ";	// to do: do something smarter
				break;
	}

	USES_CONVERSION_U8;
	fout << T2CU8(m_sFirst) << " " << T2CU8(m_sSecond) << " ";
...
void CWTest::write(ostream& fout, char cCommentChar) const
{
	...
	USES_CONVERSION_U8;
	fout << "\\" << T2CU8(((CWTestList*)m_pOwningList)->getSFMCode()) << " ";
	...
	fout << " " << T2CU8(getNStringFromNRString(m_sContents)) << " ";
	fout << "\n";
}
...
void CWListRowItem::writeCommentStr(ostream & fout, const CString & sComment, char cCommentChar)
{
	...
	LPTSTR x = (LPTSTR) sC.GetBuffer(1);
	USES_CONVERSION_U8;
	while(*x)
	{
		fout << TCH2CU8(*x);
...
{
	CString s;
	s = _T("VER0 ");	// which version of this field
	for(int i=0; i< m_iNumColumns; i++)
	{
		...
		if(m_iColumnWidths.GetSize() <= i)
			...
			n.Format(_T("%d "), m_iColumnWidths[i]);
		s+= n;
	}
	return s;
}
...
{
	insertColumn(clc, _T("Infix Location"), 100);
	insertColumn(clc, _T("Type"), 50);
	insertColumn(clc, _T("Morphname"), 150);
	insertColumn(clc, _T("Allomorph"),	100);
}
