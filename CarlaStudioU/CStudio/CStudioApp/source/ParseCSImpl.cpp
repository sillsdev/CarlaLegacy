								   ...
								   BOOL bForceRefresh, BSTR *pbstrResult)
{
	...
	try
	{
		...
		CString sResult = m_pPhraseParser->parseString(OLE2T(sPhrase), bTrace, OLE2T(sTraceWords), bForceRefresh);
		...
		{
			sThrow.Format(_T("Could not understand: %s"), (LPCTSTR)sResult);
			return Error (sThrow);
		}
...
STDMETHODIMP CParseCSImpl::ParseFile(BSTR bstrInputFilePath, BSTR bstrOutputFilePath, BOOL bForceRefresh)
{
	...
	if(m_bstrSourceLangCode.Length == 0)
		return Error (_T("The language code was not set before calling Execute()"));

	if(!m_pPhraseParser)
		return Error (_T("CS bug: Somehow the PhraseParser was not created before calling Execute()"));
...
/*		CString sInputFilePath = OLE2A(bstrInputFilePath);
		...
		CString sResult;
*/
		CString sResult= m_pPhraseParser->parseFile(OLE2T(bstrInputFilePath), OLE2T(bstrOutputFilePath), bForceRefresh);
...
CCarlaLanguage* CParseCSImpl::getLangFromCode(BSTR bstrLangCode)
{
	...
	{
		sThrow = (_T("Somehow CarlaStudio was handed an empty language code string"));
		throwIt;
	}
...
STDMETHODIMP CParseCSImpl::get_LangInfoXML(BSTR *pVal)
{
	...
	for(int i = 0; i<vecLangDocs.GetSize(); i++)
	{
		...
		CCarlaLanguage *pL = pDoc->getLang();
		CString sLang;
		sLang.Format(_T("<lang id='%s' name='%s' abrev='%s' ctrlFilesDir='%s'>"),
					(LPCTSTR)pL->getUniqueID(),
					(LPCTSTR)pL->getName(),
					(LPCTSTR)pL->getAbrev(),
					(LPCTSTR)pL->getMFS()->getCtrlFilesDirectory());

		const CTextDisplayInfo* pD = pL->getDisplayInfo();
		ASSERTX(pD);
		CString s;
		s.Format(_T("<display morphnamesInLangFont='%s'>"), pD->m_bShowMorphnamesInLangFont?_T("y"):_T("n"));
		sLang+= s;

			CFont* pF = pD->getFont();
			LOGFONT lf;
			pF->GetLogFont(&lf);
		s.Format(_T("<font faceName='%s' />") , lf.lfFaceName);
		sLang+= s;
		sLang+= _T("</display>");
		sLang += _T("</lang>");
		sOut += sLang;
