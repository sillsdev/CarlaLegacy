...
//
//changes
// rde270 18-Jun-2002 switch to RichEdit 3.0 for switch to unicode.
...
CCarlaStudioApp::CCarlaStudioApp()
  :m_recentProjectsList(14, _T("Recent Projects"), _T("File%d"), kMaxProjectsInMRU) // increase index so 1st project number is not same as last document number
#ifndef rde270
  , m_hmodRichEd1( NULL )
#endif  // rde270
{
...
#ifndef rde270
void CCarlaStudioApp::FreeRichEdLibrary()
{
	if( m_hmodRichEd1 != NULL )
		FreeLibrary( m_hmodRichEd1 );
}
#endif  // rde270
/////////////////////////////////////////////////////////////////////////////
...
{
#ifndef rde270
	// for RichEdit 3.0, you have to manually load the correct DLL.
	m_hmodRichEd1 = LoadLibrary(_T("riched20.dll"));
#else   // rde270
	AfxInitRichEdit( );
#endif  // rde270
...
*/
	...
	{
#ifdef  _UNICODE
		CGuruSplash * pSplashWnd = new CGuruSplash(IDB_SPLASHU,2000);
#else   // !_UNICODE
...
//	OnAppAbout();
	...
	m_sLastOpenDir = GetProfileString( _T("Defaults"), _T("LastOpenDir"));

	CString sProjectPath = GetProfileString( _T("Defaults"), _T("LastProjectFile"));
...
//	pMainFrame->UpdateWindow();
	...
	catch(LPCTSTR lpszError)
	{
		...
		s += _T("This is definitely a bug in CarlaStudio.\n");
#ifndef hab15a3
		s += _T("Please send a bug report, and include the following information:\n\n"); // hab 09/09/1999 fix typo
#else
		s += _T("Please sent a bug report, and include the following information:\n\n");
#endif // hab15a3
		CString sWhere;\
		sWhere.Format(_T("Uncaught LPTSTR Exception \"%s\""), lpszError);
		s += sWhere;
...
#endif // hab15a3
		CString sWhere;\
		sWhere.Format(_T("Uncaught CString Exception \"%s\""), (LPCTSTR)sError);
		s += sWhere;
		AfxMessageBox(s, MB_ICONSTOP | MB_OK);
		return FALSE;
	}
...
CProjectDoc* CCarlaStudioApp::getProject(BOOL bDontComplainIfNone)
{
	...
	else if(!bDontComplainIfNone)
	{
		...
		//jdh 10 July 200 added this msg, removed assert
		 AfxMessageBox(_T("You must open or create a project before doing that."));
	}
...
CDocument* CCarlaStudioApp::OpenDocumentFile(LPCTSTR lpszFileName)
{
	...
	*/
	...
	// Is it an input doc??
	else if(sExt.Left(2) == _T("an")) // permit ana, anb
	{
		if(!getProject(TRUE))
		{
			AfxMessageBox(_T("You must open a project before opening other document types."));
			return NULL;
		}
//		pDoc =  CWinApp::OpenDocumentFile(lpszFileName);
...
BOOL CCarlaStudioApp::askUserToFindFile(CString & sPath, LPCTSTR lpszRegFileTypeID)
{
	...
	CString s;
	CString sMsg(_T("CarlaStudio couldn't open the "));
	sMsg += sFileTypeName;
	sMsg += _T(" named: ");
	sMsg += getFullFileName(sPath);
	sMsg += _T(", which it expected to find at ");
	sMsg += sPath;
	sMsg += _T("\n\nThis can happen if you've copied this project from a different computer or moved files around on your computer.\n\n");
	sMsg += _T("After you choose OK, CarlaStudio will let you locate this file.");
	MessageBox( NULL, sMsg, _T("File not found"),  MB_ICONQUESTION | MB_OK);
	...
	CFileDialog dlg( TRUE,
		...
		s, NULL );
	CString sPrompt(_T("Please locate the file: "));
	sPrompt += getFullFileName(sPath);

	LPTSTR lpszPrompt = new TCHAR[sPrompt.GetLength()+1];
	_tcscpy(lpszPrompt, sPrompt);
...
void CCarlaStudioApp::OnFILEOPENDocument()
{
	...
	//		 20-June-2000 jdh added anx as a valid ana file extension
	CString sFilter(_T("Source Texts And ANA Files (*.txt; *.ana; *.anx)|*.txt; *.ana; *.anx|Other text files (*.*)|*.*||"));
...
#ifndef rde270
	// clear the global in case the user next opens a differently encoded project.
	ResetUTF8Safeness();
#endif  // rde270
}
...
void CCarlaStudioApp::AddToRecentFileList(LPCTSTR lpszPathName)
{
	...
	if(s.Right(3) == _T("txt") || s.Right(3) == _T("ana")		// SOURCE & ana TEXTS
		|| s.Right(3) == _T("anx")) // jdh 20June2000 added
	{
		CWinApp::AddToRecentFileList(lpszPathName);
	}
	else if(s.Right(3) == _T("csp"))	// PROJECTS
	{
...
void CCarlaStudioApp::OnFileMruProject(UINT id)
{
	...
	TRACE(_T("PROJECT MRU: open file (%d) '%s'.\n"), (nIndex) + 1,
...
int CCarlaStudioApp::ExitInstance()
{
	...
	{
		WriteProfileString( _T("Defaults"), _T("LastOpenDir"), m_sLastOpenDir);
	}
								   ...
								   /*in*/		CDocument *pDoc/*=NULL*/)
{
	...
	if(pDoc)
	{
		...
		{
				AfxMessageBox(_T("CarlaStudio does not know how to process this file."));
				return kBadDoc;
		}
	...
	{
		AfxMessageBox(_T("You must first open a document you want to process."));
...
*/
	...
	CString sTempDocsDir;
	sTempDocsDir.Format(_T("%s\\CSTUDIO"), _tgetenv(_T("TEMP")));


	CreateDirectory(sTempDocsDir, NULL);// MAKE SURE IT IS THERE
	sTempDocsDir += _T("\\TempDocs");
	CreateDirectory(sTempDocsDir, NULL);// MAKE SURE IT IS THERE

	// find a name that isn't already in use as an open document
	for(int i=1; ; i++)
	{
		sTry.Format( _T("%s\\doc%d.txt"), (LPCTSTR)sTempDocsDir, i);
	...
	USES_CONVERSION;
	FILE * tempFile = 	fopen(T2CA(sTry), "w");
	fclose(tempFile);
...
void CCarlaStudioApp::OnJump()
{
	...
	if(!pv)
		return;
#ifndef rde269
	if( pv->IsKindOf(RUNTIME_CLASS(CInputDocView)))
		p = ((CInputDocView*)pv)->GetSelCtrl();
	else
		p = pv->GetFocus();
#else
	p = pv->GetFocus();
	...
	CString s;
	if(p->IsKindOf(RUNTIME_CLASS(AsyncLoadRichEditCtrl)))
	{
		AsyncLoadRichEditCtrl* pRec = ((AsyncLoadRichEditCtrl*)p);
		long lStart, lEnd;
...
#endif  // rde269
		if(s.IsEmpty())
			AfxMessageBox(_T("You must select a word first."));
		else
