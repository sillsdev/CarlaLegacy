...
//
// rde269   25-Jun-2002 Delete the CPrefsInputDocDisplay created new in the ctor if we get a
//                      OnNewDocument (or it'll leak).
#include "stdafx.h"
...
{
#ifndef rde269
	// delete the CPrefsInputDocDisplay if it previously existed (new'd in ctor)--it was leaking
	if( m_pInputDocDisplayPrefs )
		delete  m_pInputDocDisplayPrefs;
#endif  // rde269
	...
	{	AfxMessageBox(_T("You must close the open project before you can create a new one"));
...
*/
	...
	if(!ok)
	{
		...
		MessageBox( NULL, _T("No Project Created"), _T("Note:"), MB_OK);
		return FALSE;
	}
	else
	{
		...
		if(sProjectPath.IsEmpty())
			AfxMessageBox(_T("We recommend that you create a new folder to store this project and its files in."));
...
//	ofstream fout(lpszPathName, ios::out );
	...
	AfxGetApp()->WriteProfileString( _T("Defaults"), _T("LastProjectFile"), lpszPathName);
...
{
	if(this != theApp.getProject())
	{	AfxMessageBox(_T("You must close the open project before you can open another one"));
		//gpProgressBar = NULL;
		return FALSE; // can only have one project at a time;
	}
...
/*	CDlgProgress dlg;
	...
	dlg.SetPos(0);
	dlg.SetStatus(_T("Opening project..."));
*/
	CSimpleProgressBar progressBar(_T("Reading Project File..."));;
	...
	USES_CONVERSION;
	SFMFile f(T2CA(lpszPathName), '*'); // comment won't be found in this kind of file
	if(!f.isOpen())	// that's ok, the file could have been deleted since the last run
	{
		CString s(_T("Couldn't open the Project File named:"));
		s += lpszPathName;
	...
	try
	{
		...
		{
			if((bFirst &&  sMarker != _T("id")) || sMarker == _T("formatVersion"))
			{
				bFirst = FALSE;
				int iVersion = -1;
				if(sMarker == _T("formatVersion"))
				{
					...
					iVersion = _ttoi(sField);
				}
				if (iVersion < FORMAT_VERSION)
				{
					AfxMessageBox(_T("Sorry, the format of this project file is no longer supported.  No data will actually be lost, however.  You need to create a new project in a different folder, then make a new language using your existing control files. Ah, the joys of alpha testing..."));
	...
	progressBar.SetText(_T("Reading Language File..."));
	...
	progressBar.SetText(_T("Loading Source Document..."));
			...
			{
				//ASSERTX(getSourceLangDoc());	// if this fails, it's probably because the \SrcLang marker wasn't found before this one
	progressBar.SetText(_T("Loading ANA Document..."));
				CAnaInputDoc::loadDocument(sField, f);
			}
			else
				f.logSkip(_T("Project"), sMarker, sField);
		}
	}
...
{
	CPropertySheet dlg(_T("Project Settings"));
		...
		if(sLabel==_T("&Project"))
		{
			...
			for( int iLang=0; iLang<m_pLangDocs.GetSize(); iLang++)
			{
				...
				subMenu.InsertMenu(0, flags,
					LANG_MENU_COMMAND_BASE + (LANG_MENU_NUM_ITEMS*iLang) + 0, _T("Show Model Window"));
				subMenu.InsertMenu(1, flags,
					LANG_MENU_COMMAND_BASE + (LANG_MENU_NUM_ITEMS*iLang) + 1, _T("Properties..."));
				subMenu.InsertMenu(2, flags,
					LANG_MENU_COMMAND_BASE + (LANG_MENU_NUM_ITEMS*iLang) + 2, _T("Analysis Sequence..."));
				...
				subMenu.InsertMenu(iIndex, flags,
					LANG_MENU_COMMAND_BASE + (LANG_MENU_NUM_ITEMS*iLang) + iIndex, _T("Synthesis Sequence..."));
...
{
	ASSERTX(lpszLangID && _tcslen(lpszLangID));
	for( int i=0; i<m_pLangDocs.GetSize(); i++)
	{
...
void CProjectDoc::OnFILENewLanguage()
{
	...
	{
		AfxMessageBox(_T("Sorry, the maximum number of languages has been reached.  Please write to carlastudio_support@sil.org to have the maximum increased."));
		return;
	}
...
void CProjectDoc::cleanupAfterOpen()
{
	...
	ce.ReplaceSel(_T("This window represents your project, but has no purpose at this time.  My apologies for the nuisance."));
...
//	put something in there so that until we can truely hide the thing, it isn't empy
   ...
   pView = GetNextView(pos);
	...
	ASSERTX(pView->IsKindOf(RUNTIME_CLASS(CEditView)));
	CEdit& ce  = ((CEditView*)pView)->GetEditCtrl();
	ce.ReplaceSel(_T("Please ignore this window."));
	SetModifiedFlag(FALSE);  // don't want this message to make the whole project dirty
...
CCarlaLanguage* CProjectDoc::getLangFromIndex(int i)
{
	...
	{
		AfxMessageBox(_T("This project thinks that it doesn't have any languages installed.  Please report this bug.  If you receive this message as soon as you run CarlaStudio, the default project may be corrupt. Find CStudio.ini and delete it.  Next, run CarlaStudio again and create a new project."));
