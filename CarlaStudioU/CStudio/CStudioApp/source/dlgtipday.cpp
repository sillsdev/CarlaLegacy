...
#include <sys\types.h>
#include "DlgTipDay.h"
#include "PathDescriptor.h" // for get directory
...
CDlgTipDay::CDlgTipDay(CWnd* pParent /*=NULL*/)
	: CDialog(IDD_TIP, pParent)
{
	//{{AFX_DATA_INIT(CDlgTipDay)
	m_bStartup = TRUE;
	//}}AFX_DATA_INIT
	...
	if(!m_pStream)	// so I don't have to keep the tips in the DEBUG directory
		m_pStream= fopen("../tips.txt", "r");
	if (m_pStream == NULL)
	{
	...
	USES_CONVERSION;
	CString strCurrentTime = A2CT(ctime(&buf.st_ctime));
...
void CDlgTipDay::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CDlgTipDay)
	DDX_Check(pDX, IDC_STARTUP, m_bStartup);
	DDX_Text(pDX, IDC_TIPSTRING, m_strTip);
	//}}AFX_DATA_MAP
}

BEGIN_MESSAGE_MAP(CDlgTipDay, CDialog)
	//{{AFX_MSG_MAP(CDlgTipDay)
...
/////////////////////////////////////////////////////////////////////////////
// CDlgTipDay message handlers

void CDlgTipDay::OnNextTip()
{
	GetNextTipString(m_strTip);
	UpdateData(FALSE);
}

void CDlgTipDay::GetNextTipString(CString& strNext)
{
...
HBRUSH CDlgTipDay::OnCtlColor(CDC* pDC, CWnd* pWnd, UINT nCtlColor)
{
