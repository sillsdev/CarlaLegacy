...
:CWModel(_T("STAMPTRANSFER")), // this label currently unused, actually
 m_lexChanges(pTDI),
...
BOOL CStampTransferModel::loadFromFile(LPCTSTR lpszPathName, CWCommonModel& commonModel,  BOOL bLogIfMissing)
{
	...
	if(!f.ensureOpen(_T("STAMP Transfer File"), bLogIfMissing))
		return FALSE;
	...
	try
	{
		while(f.getField(sMarker, sField, &bEnabled))
		{
			try
			{
				...
				else if(sMarker == _T("sr") || sMarker == _T("cr") || sMarker == _T("ir"))
				{
#ifndef hab17a1
				...
				else if(sMarker == _T("CarlaStudioLexChangePrefs"))
					m_lexChanges.readPrefsField(sField);
				else if(sMarker == _T("CarlaStudioXferExprPrefs"))
...
#endif //hab17a1
#ifndef hab211
				else if(sMarker == _T("CSbegtr"))
				{
				  pLastList = &m_transferExpressions;
				}
				else if(sMarker == _T("CSbeglc"))
				{
				...
				{	f.logSkip(_T("STAMP-Transfer"), sMarker, sField);
					//TRACE(_T("CStudio STAMP-Transfer parser Skipping Field [%s]\n"), sMarker);
				}
			}
			catch(CString sProblemStr) // these ones we can keep going
			{
				f.logParseFailure(_T("STAMP-Transfer"), sMarker, sField);
			}
		}
	}
	catch(CString sProblemStr)// this one catches problems encountered by getField()
	{
		CString s(_T("CStudio STAMP-Transfer control file parser could not parse: "));
		s += sProblemStr;
	...
	commentCollector.defaultDisgorge(m_lexChanges, _T("STAMP Lexical Changes"), lpszPathName);
#endif // hab17a1
...
#ifndef hab211
	m_transferExpressions.write(fout, commonModel.m_cCommentChar, _T("tr"));
	m_lexChanges.write(fout, commonModel.m_cCommentChar, _T("lc"));
#else //hab211
	m_transferExpressions.write(fout, commonModel.m_cCommentChar, _T("tr"));
	m_lexChanges.write(fout, commonModel.m_cCommentChar, _T("lc"));
#endif //hab211
// handled here instead of by carlalanguage because of multiple transfer sets
	USES_CONVERSION_U8;
	fout << "\\CarlaStudioLexChangePrefs " << T2CU8(m_lexChanges.getPrefsField()) <<'\n';
	fout << "\\CarlaStudioXferExprPrefs " << T2CU8(m_transferExpressions.getPrefsField()) <<'\n';
...
{
	return _T("Stamp Morpheme Transfer Rule");
}

// called by CWListView::OnInitialUpdate
void CStampTransferExpressionList::insertColumns(CListCtrl &clc)
{
	insertColumn(clc, _T("Comment"),	300);
	insertColumn(clc, _T("Environment"),	100);
	insertColumn(clc, _T("Flags"), 75);
	insertColumn(clc, _T("Substitute"), 100);
	insertColumn(clc, _T("Match/Insert"), 100);
...
CString CStampTransferRule::getText(int iColumn)
{
	...
	switch(iColumn)
	{
		 case 0: switch(m_iType)
				 {
					case kSTRSub: s= _T("Subst"); break;
					case kSTRCopy: s= _T("Copy"); break;
					case kSTRInsert: s= _T("Insert"); break;
	...
	switch(m_iType)
	 {
		case kSTRSub: fout << "\\sr ";
				fout << "\"" << T2CU8(m_sFrom) << "\" \"" << T2CU8(m_sTo) << "\" " << T2CU8(m_sFlags) << ' ';
				break;
		case kSTRCopy: fout <<  "\\cr ";
				fout << "\"" << T2CU8(m_sFrom) << "\" \"" << T2CU8(m_sTo) << "\" " << T2CU8(m_sFlags) << ' ';
				break;
		case kSTRInsert: fout <<  "\\ir ";
				fout << "\"" << T2CU8(m_sFrom) << "\" " << T2CU8(m_sFlags) << ' ';
				break;
		default: ASSERTX(FALSE); break;
	 }
...
CString CStampTransferFlag::getText(int iColumn)
{
	...
	switch(iColumn)
	{
		 ...
		 case 1: return m_sMorphemes;
				break;
		 case 2: return _T(""); // subst/insert
				break;
		 case 3: return m_sFlag;
				break;
		 case STAMP_TRANSFER_RULE_ENVIRONCOLUMN: s= _T(""); // this is the env column for transferRules, not used for flags
				break;
		// comment column
...
{
	USES_CONVERSION_U8;
	fout << "\\fl " << T2CU8(m_sFlag) << ' ' << T2CU8(m_sMorphemes);
...
{
	USES_CONVERSION_U8;
	fout << "\\lc \"" << T2CU8(m_sFrom) << "\" > \"" << T2CU8(m_sTo) << "\" ";
	CWEnvironConstrainedRule::write(fout, cCommentChar);
 }
...
const CString CStampLexChangeList::getDisplayName() const
{
	  ...
	  return _T("Synthesis Lexical Change");
	else
	  return _T("Stamp Lexical Change");
#else // hab17a1
	return _T("Stamp Lexical Change");
#endif // hab17a1
}
...
#endif // hab17a1
	m_tests(_T("test"), pTDI),	// this is the SFM marker used when writing ctrl file
	CWModel(_T("STAMPSYNTHESIS")),
	m_protoformChars(_T("Protoform Chars"), _T(""), pTDI)
{
}
										 ...
										 BOOL bLogIfMissing)
{
	...
	if(!f.ensureOpen(_T("STAMP Synthesis File"), bLogIfMissing))
		return FALSE;
	...
	try
	{
		while(f.getField(sMarker, sField, &bEnabled))
		{
			try
			{
				...
				else if(sMarker == _T("rscid"))
				{
					m_protoformChars.setData(sField);
				}

				else if(sMarker == _T("test"))
				{
#ifndef hab17a1
				...
				else if(sMarker == _T("CSbegrsc"))
				{
				  pLastList = &m_regSoundChanges;
				}
				else if(sMarker == _T("CSbegtest"))
				{
				  pLastList = &m_tests;
				}
				else if(sMarker == _T("CSend")) // CS-specific section end marker
				...
				{	f.logSkip(_T("STAMP-Synthesis"), sMarker, sField);
					TRACE(_T("CStudio STAMP-Synthesis parser Skipping Field [%s]\n"), sMarker);
				}
			}
			catch(CString sProblemStr) // these ones we can keep going
			{
				f.logParseFailure(_T("STAMP-Synthesis"), sMarker, sField);
			}
		}
	}
	catch(CString sProblemStr)// this one catches problems encountered by getField()
	{
		CString s(_T("CStudio STAMP-Synthesis control file parser could not parse: "));
		s += sProblemStr;
		MessageBox( NULL, s, _T("Parsing Error"), MB_OK);
		return FALSE;
	}
...
#else // hab17a1
	// we have to guess where these go these unclaimed comments
	commentCollector.defaultDisgorge(m_regSoundChanges, _T("STAMP Protoform Rules"), lpszPathName);
#endif // hab17a1
	return TRUE;
}
...
BOOL CStampSynthesisModel::writeFile(CString& sPath, CWCommonModel& commonModel)
{
	...
	m_regSoundChanges.write(fout, commonModel.m_cCommentChar, _T("rsc"));
	m_tests.write(fout, commonModel.m_cCommentChar, _T("test"));
#else // hab211
#ifndef hab17a1
...
void CRegSoundChangeList::insertColumns(CListCtrl &clc)
{
	...
	insertColumn(clc, _T("Comment"),	300);
	insertColumn(clc, _T("Environment"),	100);
	insertColumn(clc, _T("RSC ids"),	50);
	insertColumn(clc, _T("To"),	50);
	insertColumn(clc, _T("    From"), 80);	// space are so it isn't over the icons
#else // hab17a1
	insertColumn(clc, _T("Comment"),	300);
	insertColumn(clc, _T("Environment"),	100);
	insertColumn(clc, _T("To"),	50);
...
void CRegSoundChange::write(ostream& fout, char cCommentChar) const
{
	USES_CONVERSION_U8;
#ifndef hab17a1
		fout << "\\rsc \"" << T2CU8(m_sFrom) << "\" \"" << T2CU8(m_sTo) << "\" " << T2CU8(m_sRscIds) << "\n";
#else // hab17a1
...
: m_sRscIds(_T("")), CWEnvironConstrainedRule(3, bEnabled)
{
