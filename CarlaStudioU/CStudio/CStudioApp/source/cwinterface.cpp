...
static void addTransferModel(CTransferModelSet* pTMS, CWTopicTab* pTab, LPCTSTR lpszLabel)
{
	...
	pTab->Add(pHeading);
		pFTopic = new CWFormTopic(pTab, _T("SENTRANS Punct"));
...
#ifndef hab17a1
	pHeading->Add(new CWListTopic(pTab, _T("SENTRANS CL Field"), &rSTModel.m_genericClasses));
	pHeading->Add(new CWListTopic(pTab, _T("SENTRANS Patterns"), &rSTModel.m_patterns));  // jdh 5/29/01
	pHeading->Add(new CWListTopic(pTab, _T("SENTRANS Rules"), &rSTModel.m_rules));
#else // hab17a1
	pHeading->Add(new CWListTopic(pTab, _T("SENTRANS Rules"), &rSTModel.m_rules));
	pHeading->Add(new CWListTopic(pTab, _T("SENTRANS CL Field"), &rSTModel.m_genericClasses)); // jdh 11/9/99
#endif // hab17a1

	// stamp transfer
	pHeading->Add(new CWListTopic(
					pTab,
					_T("STAMP Morpheme Transfer"),
					&pTMS->getStampTransferModel()->m_transferExpressions));
	pHeading->Add(new CWListTopic(
					pTab,
					_T("STAMP Lexical Changes"),
...
void CWInterface::populateDictTab()
{
	...
	m_pDictTab = new CWTopicTab(_T("Dictionary") );
	...
	{
#ifndef hab17a1
		pFTopic = (CWFormTopic*) m_pDictTab->Add(new CWFormTopic(m_pDictTab, _T("Analysis SFM Markers")));
		...
		pFTopic = (CWFormTopic*) m_pDictTab->Add(new CWFormTopic(m_pDictTab, _T("Interlinear SFM Markers")));
		pFTopic->Add(&m_sfmNotice);
		pFTopic->Add(&m_multipleAddNotice);
		pFTopic->Add(&m_multipleDelNotice);
		m_pLang->getInterCodeTable().m_unifiedSet.addSelfToTopic(pFTopic);

		pFTopic = (CWFormTopic*) m_pDictTab->Add(new CWFormTopic(m_pDictTab, _T("Synthesis SFM Markers")));
		pFTopic->Add(&m_sfmNotice);
		pFTopic->Add(&m_multipleAddNotice);
		pFTopic->Add(&m_multipleDelNotice);
		m_pLang->getSynthCodeTable().m_unifiedSet.addSelfToTopic(pFTopic);
#else //hab17a1
		pFTopic = (CWFormTopic*) m_pDictTab->Add(new CWFormTopic(m_pDictTab, _T("SFM Markers")));
		pFTopic->Add(&m_sfmNotice);
	...
	{
#ifndef hab17a1
		pHeading = new CWHeadingTopic(m_pDictTab, _T("Analysis SFM Markers"));
		m_pDictTab->Add(pHeading);

		if (m_pLang->getHavePrefixes())
		  {
		pFTopic = (CWFormTopic*) pHeading->Add(new CWFormTopic(m_pDictTab, _T("Prefix")));
		pFTopic->Add(&m_sfmNotice);
		pFTopic->Add(&m_multipleAddNotice);
		pFTopic->Add(&m_multipleDelNotice);
		...
		if (m_pLang->getHaveInfixes())
		  {
		pFTopic = (CWFormTopic*) pHeading->Add(new CWFormTopic(m_pDictTab, _T("Infix")));
		pFTopic->Add(&m_sfmNotice);
		pFTopic->Add(&m_multipleAddNotice);
		pFTopic->Add(&m_multipleDelNotice);
		...
		if (m_pLang->getHaveSuffixes())
		  {
		pFTopic = (CWFormTopic*) pHeading->Add(new CWFormTopic(m_pDictTab, _T("Suffix")));
		pFTopic->Add(&m_sfmNotice);
		pFTopic->Add(&m_multipleAddNotice);
		pFTopic->Add(&m_multipleDelNotice);
		m_pLang->getCodeTable().m_suffixSet.addSelfToTopic(pFTopic);
		  }
		pFTopic = (CWFormTopic*) pHeading->Add(new CWFormTopic(m_pDictTab, _T("Root")));
		pFTopic->Add(&m_sfmNotice);
		pFTopic->Add(&m_multipleAddNotice);
		pFTopic->Add(&m_multipleDelNotice);
		m_pLang->getCodeTable().m_rootSet.addSelfToTopic(pFTopic);

		pHeading = new CWHeadingTopic(m_pDictTab, _T("Interlinear SFM Markers"));
		m_pDictTab->Add(pHeading);
		...
		pHeading = new CWHeadingTopic(m_pDictTab, _T("Synthesis SFM Markers"));
		m_pDictTab->Add(pHeading);
...
#else // hab17a1
		pHeading = new CWHeadingTopic(m_pDictTab, _T("SFM Markers"));
		m_pDictTab->Add(pHeading);
	...
	pFTopic = (CWFormTopic*) m_pDictTab->Add(new CWListTopic(m_pDictTab, _T("Dict Orthography Changes"), &m_pLang->getDictOrthoModel().m_orthoChanges));

	CWAnalysisModel &analysis = m_pLang->getAnalysisModel();

	// capitalization
		pFTopic = (CWFormTopic*) m_pDictTab->Add(new CWFormTopic(m_pDictTab, _T("Dictionary Capitalization")));
	...
	pHeading = new CWHeadingTopic(m_pDictTab, _T("Phonology"));
	m_pDictTab->Add(pHeading);
		pFTopic = new CWFormTopic(m_pDictTab, _T("Settings"));
		pHeading->Add(pFTopic);
			pFTopic->Add(& phonrule.m_bBaseBecomesAllomorph);
			pFTopic->Add(& phonrule.m_cCutoffChar);
			pFTopic->Add(& phonrule.m_sCitationMarker);

		CWListTopic* pLTopic = new CWListTopic(m_pDictTab, _T("Rules"),
			&phonrule.m_rules);
