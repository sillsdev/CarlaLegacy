...
STDMETHODIMP CHook::PerformTask(VARIANT varICSTask)
{
	...
	LPTSTR lpszInPath = OLE2T(qTask->GetInputPath());
	...
	try
	{
		...
		if(!sPath.fileExists())
		{
			sThrow.Format(_T("Could not find the input file, %s"), (LPCTSTR)sPath.getFullPath());
			throwIt;
		}
		...
		{
			cmd.sDesiredOutputPath = OLE2T(qTask->GetDesiredOutputPath());
			if(cmd.sDesiredOutputPath.GetLength() == 0)
				throw _T("You must specify the desired output path");
		}
		else if(cmd.eOutputLocation == csReplaceInput)
		{
			...
			{
				cmd.sDesiredOutputPath = sPath.getDirectory()+sPath.getFileName()+_T("2")+sPath.getFileExtension();
				CString s;
				s.Format(_T("Couldn't backup %s, so the output file will not overwrite it.  Instead, the resulting file will be %s"), (LPCTSTR)sPath.getFileFullName(), (LPCTSTR)cmd.sDesiredOutputPath);
				AfxMessageBox(s);
			}
			else
		...
		if(!cmd.pSourceLang)
			throw _T("You must specify a source language");

		CString sTarLangCode = OLE2T(qTask->GetTargetLangCode());
		if(cmd.iGoal == CProcessingPrefs::kTargetText)
		{
			if(sTarLangCode.IsEmpty())
				throw _T("You must specify a target language when doing transfer");

			cmd.pTargetLang = theApp.getProject()->getLangFromID(sTarLangCode);
			if(!cmd.pTargetLang)
			{
				sThrow.Format(_T("This project does not have a language with code %s"), (LPCTSTR)sTarLangCode);
				throwIt;
			}
		}
		...
		if(qTask->GetInputIsInterlinear())	// first need to clean it (de-interlinearize it)
		{
			...
			cmd.sMarkersToIncludeOrExclude = OLE2T(qTask->GetInterlinearWordMarker());
			...
			if(cmd.sMarkersToIncludeOrExclude.IsEmpty())
				throw _T("You must specify the marker for the text line");
			...
			cmd.sMarkersToIncludeOrExclude = CString(_T("\\"))+cmd.sMarkersToIncludeOrExclude ;
		...
		else // plain 'ol text, so set the textin text markers
		{
			...
			cmd.sMarkersToIncludeOrExclude = OLE2T(qTask->GetMarkersToIncludeOrExclude());
			if(cmd.sMarkersToIncludeOrExclude.IsEmpty())
				throw(_T("You must include the text markers to include or exclude"));
		}
		...
		if(!pDoc)
		{
			sThrow.Format(_T("Couldn't open the file: %s"), (LPCTSTR)sPath.getFullPath());
			throwIt;
		}
...
{
	USES_CONVERSION;
	CString sLangCode = OLE2T(bstrLangCode);
	...
	if(!pLang)
	{
		sThrow.Format(_T("This project does not have a language with code %s"), (LPCTSTR)sLangCode);
		throwIt;
	}
...
CString CHook::deinterlinearize(CPathDescriptor& pathInput, ITask* pTask)
{
	...
	ofstream fout;
	CString sCleanedPath = pathInput.getDirectory() + _T("_") + pathInput.getFileName() + _T(".txt");
	fout.open(T2CA(sCleanedPath));
	if(!fout.is_open())
	{
		sThrow.Format(_T("Couldn't create the file for writing the deinterlinearized text: %s"), (LPCTSTR)sCleanedPath);
		throwIt;
	}
	try
	{
		...
		sTextMrk = OLE2T(qTask->GetInterlinearWordMarker());
		sTextMrk.TrimRight();
		sTextMrk.TrimLeft();
		if(sTextMrk.GetLength() && sTextMrk[0] == '\\')	// trim off leading slash
			sTextMrk = sTextMrk.Mid(1);
		if(sTextMrk.IsEmpty())
			throw _T("The Interlinear Word Marker is empty.");

		// break up the string of markers into an array of markers
		CParseStream stream(OLE2T(qTask->GetAnalysisMarkers()));
		CString sMrk;
	...
	{
		sThrow.Format(_T("While trying to deinterlinearize %s, got this error:\n%s"), (LPCTSTR)pathInput.getFullPath(), lpszError);
		throwIt;
	}
...
{
	TCHAR lpszBakPath[_MAX_PATH+1];
	//_stprintf(lpszBakPath, _T("%s.bak"), lpszInPath); // leave the old extension, but add .bak
	...
	{
		_stprintf(lpszBakPath, _T("%s.%d.bak"), lpszInPath, i);
		if(-1==_taccess(lpszBakPath, 00)) // if not found
			break;
	}
	...
	TCHAR lpszRemoveBakPath[_MAX_PATH+1];
	_stprintf(lpszRemoveBakPath, _T("%s.%d.bak"), lpszInPath, (i%nMaxNumber)+1);
	_tremove(lpszRemoveBakPath);

	// delete any old backup using this name
	_tremove(lpszBakPath);
