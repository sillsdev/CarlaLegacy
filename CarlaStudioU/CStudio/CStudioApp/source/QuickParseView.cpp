...
void CQuickParseView::getManualParse(CProcessStatus &status, CString &sPath, CCarlaLanguage* pSourceLang)
{
  ...
  while (m_pAlloChosen != NULL)
	...
	{
	  if (sState == _T("BOW") || sState == _T("PFX"))
		sState = _T("PFX");
	  else
		sState = _T("SFX");
	}
...
#endif // hab254
	{
	  ...
	  m_sTraceMorphs +=  m_pAlloChosen->m_sMorphName + _T(" ");
	  if (sState == _T("PFX"))
	{
	  sAllos  += TCHAR(cDecompSepChar);
	  sMNames += TCHAR(cDecompSepChar);
				...
				// get next allomorph
				// adjust input string
	  if (m_pAlloChosen->m_sShape == _T("0"))
...
#ifndef hab254
	  int iPos = sRemainingInput.FindOneOf(_T(" \t\n"));
	  if (iPos == -1)
...
CString &CQuickParseView::getUserChoice(CString &sAlloResults, CString &sInput)
{
  ...
  if(m_pAlloResult)
	...
	m_pAlloChosen = NULL;	// we're all done
		   ...
		   m_sResult.SpanExcluding(_T(" ")) != _T("0"))
	...
	{
	  if (iSize == 0)
		AfxMessageBox(_T("No more allomorphs were found in the dictionaries."), MB_OK);
	  else
		{
		  CQuickParseDoc* pDoc = (CQuickParseDoc*)GetDocument();
	...
	m_sResult.Format (_T("You have encountered a bug in either CarlaStudio or the Ample DLL.  CarlaStudio could not understand the Ample DLL's answer, which was: %s"), (LPCTSTR)sAlloResults);
...
*/	pStatus->setCurrentSequenceFunction(pAnalysisSequence->getFunctionCode());
	...
	if(!bOK)
		AfxMessageBox(_T("There was a problem refreshing the dictionaries.  You'll need to open a source document and parse it to see what the problem is."));
