...
{
	USES_CONVERSION_U8;
	fout << "\\ch \"" << T2CU8(m_sFrom) << "\" \"" << T2CU8(m_sTo) << "\"\n";
...
void CWMCCList::insertColumns(CListCtrl &clc)
{
	insertColumn(clc, _T("Comment"),	300);
	insertColumn(clc, _T("Environment"),	100);
	insertColumn(clc, _T("Morpheme(s)"), 100);
//	insertColumn(clc, _T(""), 50);	// enabled/disabled
}
...
: CWEnvironConstrainedRule(1, bEnabled)
{
	...
	while(stream.endOfLineComment(sTemp, TRUE))
		m_lcComments += sTemp;
		//TRACE(_T("CWMCC::CWMCC currently can't handle comments before members\n")); 	// TO DO: don't just throw these away
...
{
	return _T("Adhoc Pair");
}

/* const CString&  CWOrthoChange::getDisplayName() const
{
	static CString sName(_T("OrthoChange"));
	return sName;
}
 */
