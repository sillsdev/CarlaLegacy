...
public:
	...
	static LPCSTR ID() {return "Batch";}
	virtual LPCSTR  getProcessorID() const {return ID();}
	virtual LPCSTR getDisplayName() const { return BATCHDISPLAYNAME;}
...
#ifndef rde271c // temporarily suspend writing to log file, so the batch file can.
	void suspendProcessingLog(CProcessStatus& status);
	void resumeProcessingLog(CProcessStatus& status);
#endif  // rde271c
