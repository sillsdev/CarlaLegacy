...
: 	m_pTransferTab(NULL),
	...
	m_pDoc(pDoc),
	textMarkerGroup(_T("SFM markers of text to parse")),
	capitalizationGroup(_T("Capitalization")),
	formatGroup(_T("Format Markers")),
	wordCategoryGroup(_T("Word Category Output")),
	m_sfmNotice(_T("Enter the markers without slashes.")),
#ifndef hab17a1
	m_multipleAddNotice(_T("Use a space to add multiple markers (for items with a *).")),
	m_multipleDelNotice(_T("Leave an item blank to remove extra markers."))
#else // hab17a1
	m_mutltipleAllosNotice(_T("Use a space to add multiple allomorph markers."))
#endif //hab17a1
{}
...
void CWCarlaMenuInterface::setupForLang(CCarlaLanguage* pLang)
{
	...
	m_label = _T("CARLAMenu Organization");
	...
	CWTopicTab* pTab = new CWTopicTab(_T("Text") );
	...
	// TEXT IN CHANGES
		CWListTopic* pLTopic = new CWListTopic(pTab, _T("Input Text Changes"),
			& txtIn.m_orthoChanges);
		pHeading->Add(pLTopic);


	// TEXT OUT CHANGES
		pHeading->Add(new CWListTopic(pTab, _T("Output Text Changes"),
...
*/
	...
	pTab = new CWTopicTab(_T("Lists") );
	m_topicTabs.Add(pTab);

	pTab->Add(new CWListTopic(pTab, _T("Categories"), &common.m_categories));
	pTab->Add(new CWListTopic(pTab, _T("Allomorph Properties"), &common.m_allomorphProperties));
	pTab->Add(new CWListTopic(pTab, _T("Morpheme Properties"), &common.m_morphemeProperties));
...
*/
	pTab->Add(new CWListTopic(pTab, _T("Category Classes"), &common.categoryClasses));
	pTab->Add(new CWListTopic(pTab, _T("Morpheme Classes"), &common.morphemeClasses));
	pTab->Add(new CWListTopic(pTab, _T("String Classes"), &common.stringClasses));

	// 1.06 hab added
	pTab->Add(new CWListTopic(pTab, _T("Punctuation Classes"), &common.punctuationClasses));

	pTab->Add(new CWListTopic(pTab, _T("Adjust Gloss CL (Sentrans)"), &stGlossAdjust.m_genericClasses)); // jdh 11/9/99
	pTab->Add(new CWListTopic(pTab, _T("Adjust Analysis CL (Sentrans)"), &stAnaAdjust.m_genericClasses)); // jdh 11/9/99

	/////////////////// Constraints //////////////////////////////
	CWAnalysisModel &analysis = m_pLang->getAnalysisModel();

	pTab = new CWTopicTab(_T("Constraints") );
	m_topicTabs.Add(pTab);
	pTab->Add(new CWListTopic(pTab, _T("Morpheme Cooccurrences"), &analysis.generalMCCs));

		// sentrans disambig
	pFTopic = (CWFormTopic*) pTab->Add(new CWFormTopic(pTab, _T("Surrounding Words Punctuation")));
	...
	pTab->Add(new CWListTopic(pTab, _T("Surrounding Words Rules"), &stDisambig.m_rules));
	...
	pTab->Add(new CWListTopic(pTab, _T("Compound Root Pairs"), &analysis.compoundRootCatPairs));
	pTab->Add(new CWListTopic(pTab, _T("Ad-hoc Pairs"), &analysis.adhocPairs));


	pHeading = new CWHeadingTopic(pTab, _T("Custom Test"));
	pTab->Add(pHeading);
...
//	m_iTransferTabIndex  = m_topicTabs.Add(m_pTransferTab);
	...
	/////////////////// MISC //////////////////////////////
	pTab = new CWTopicTab(_T("Misc") );
	m_topicTabs.Add(pTab);
	pFTopic = (CWFormTopic*) pTab->Add(new CWFormTopic(pTab, _T("Maximums")));
	...
	// ana output
	pFTopic = (CWFormTopic*) pTab->Add(new CWFormTopic(pTab, _T("ANA File Output")));
...
void CWCarlaMenuInterface::populateTransferTab()
{
	...
	CWHeadingTopic* pHeading = new CWHeadingTopic(m_pTransferTab, _T("Synthesis"));
	...
	CWFormTopic* pFTopic = new CWFormTopic(m_pTransferTab, _T("Protoform Symbols"));
