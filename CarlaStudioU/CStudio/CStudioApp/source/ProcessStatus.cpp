...
*/
	...
	//jdh 11/11/99 changed to use the TEMP environment variable instead of assuming it's in the windows dir (per Randy Regnier)
	CString sRootTempDir;
	sRootTempDir.Format(_T("%s\\CSTUDIO"), _tgetenv(_T("TEMP")));
	...
	for(int i=0; i<MAXTRIES; i++)
	{
		...
		sTempFilesDirectory += _T("\\");
		sTempFilesDirectory += lpszName;

		sTempFilesDirectory += (_T("0") + i);
	...
	{
		CString s(_T("The directory the CStudio uses for temporary storage, "));
#ifdef Before1_04
		s += ROOTTEMPDIR;
#else  // hab 1.04
		s += sRootTempDir;
#endif // Before1_04
		s += _T(" has files which CStudio cannot delete.  This can happen when a processor runs into unexpected problems. You should report this situation if it continues. Quitting CarlaStudio and running it again will usually solve the problem.  If it doesn't, then restarting your computer should allow you to delete these directories.\n");
		AfxMessageBox(s);
		if(!bDirOK)
			return FALSE;
	}

	LPCTSTR x = LPCTSTR(sTempFilesDirectory);
...
void CProcessStatus::progress(LPCTSTR lpszMessage, int iPercentage)
{
	...
	{
		throw(CProcessFailure(NULL, _T("User Cancelled Processing")));
	}
	if(!lpszMessage)
