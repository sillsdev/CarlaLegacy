...
void CFocuser::focusShbxOnWord(LPCTSTR lpszWord)
{
	...
	sData.Format(_T("%s\n%s"), lpszWord, _T("surface"));

#ifndef rde280
	// Toolbox being a non-unicode (or non-wide anyway) app needs to see this as 8-bit UTF8 chars
	USES_CONVERSION_U8;
	LPCSTR lpszData = T2CU8(sData);
	...
	long lReturn = ::RegSetValue(HKEY_CURRENT_USER,
								_T("Software\\SantaFe\\Focus\\Word"),
								REG_SZ,
								sData,
								sData.GetLength());
#endif
	if ( lReturn != ERROR_SUCCESS )
		AfxMessageBox(_T("There was an error while putting this word into the registry"));
