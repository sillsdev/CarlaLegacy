...
void CJoinCompProcess::readParametersFromSFMFile(SFMFile *f)
{
	...
	while(f->getField(sMarker, sField, &bEnabled))
	{
		...
		{
			f->throwParseFailure(_T("JOINCOMPProcess"), sMarker, sField);
			//MSG2(_T("JOINCOMPProcess doesn't understand the field: "), sField);
			//throw(_T("Couldn't read the Process Sequence from the project file."));
		}
	}
}
...
void CJoinCompProcess::writeToStream(ostream& fout) const
{
	...
	if(m_sVariantFormMarker.GetLength())
		fout << "\\VariantFormMarker " << T2CU8(m_sVariantFormMarker) << "\n";

	if(m_sJoinWordsMarker.GetLength())
		fout << "\\JoinWordsMarker " << T2CU8(m_sJoinWordsMarker) << "\n";

	if(m_sAllomorphMarker.GetLength())
		fout << "\\AllomorphMarker " << T2CU8(m_sAllomorphMarker) << "\n";
...
void CJoinCompProcess::processRAWTextFile(CProcessStatus& status)
{
	...
	try
	{
		CString sBaseName = status.m_sFileNameRoot + _T("-joincomp");
		sLOGPath = status.makeTempPath(sBaseName, _T(".log"));
			//INPUTFILE.getFileName(), _T(".jnc"));
		...
		CPathDescriptor sOutputRawTextPath = status.makeTempPath(sBaseName,_T(".txt"));
			...
			iRootFileIndex++)
		{
			...
			if (iRootFileEnabled > 0)
			  {
				...
				_itot(iRootFileIndex, cNum, 10);
				sOutputTextPath = status.makeTempPath(sBaseName + cNum, _T(".txt"));
			  }
			USES_CONVERSION_U8;
			BOOL OK =  setupProcess(lpszErrorStr,
					...
					T2CU8(m_sJoinWordsMarker), //const char* lpszWordMarker,
					T2CU8(m_sAllomorphMarker), // const char* lpszAlloMarker,
					T2CU8(m_sVariantFormMarker), //const char* lpszVarMarker,

					// don't need short paths here, could be changed to long
					T2CA(status.m_pSrcDicts[iRootFileEnabled]->getFullPath()),
					T2CA(pSourceMFS->getIntxFullPath()),
					T2CA(sInputTextPath.getFullPath()),
					T2CA(sOutputTextPath.getFullPath()),
					T2CA(sLOGPath.getFullPath()));
			if(!OK)
				throw(CProcessFailure(this, U82CT(lpszErrorStr)));
	...
	WIN32_FIND_DATA fileInfo;
			if(INVALID_HANDLE_VALUE == FindFirstFile(sOutputTextPath.getFullPath(), &fileInfo))
				throw(CProcessFailure(this, _T("JOINCOMP did not produce the expected src text file.  Read the transcript for more information.")));
		...
		CResultStreamFile* txtStream =
			new CResultStreamFile(
					new CResultStreamDescriptor(this,
												_T("Joined Src Text"), //short description
												_T("Source text with words in a phrase joined by underscores"), // long description
												_T("Joined Src Text") // tab label
											),
