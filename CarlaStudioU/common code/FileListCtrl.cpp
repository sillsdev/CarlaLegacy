...
void CFileListCtrl::populate()
{
	...
	insertColumn(_T("Path"), kPathColumnWidth);
	DeleteAllItems();
...
void CFileListCtrl::insertItem(CPathDescriptor *pD)
{
	...
	if(GetDC()->GetTextExtent(sPath).cx > nColumnWidth)
		sPath = pD->getDisplayPath(int((nColumnWidth/(1.3*GetDC()->GetTextExtent(_T(" ")).cx)) ));

	lvi.pszText = (LPTSTR) LPCTSTR(sPath);
