...
CWAtomic::CWAtomic(const char* pLabel): m_label(pLabel), m_pLabel_ctrl(NULL)
{
};
...
int CWString::AddToView(int left,int top,  int labelWidth, CWnd* parent, UINT id)
{
	...
	m_pLabel_ctrl->Create( m_label + ":", WS_CHILD | WS_VISIBLE,
				labelRect, parent, 0 );
...
{
	return fout << getNStringFromNRString(i.m_data);
}

///////////////////////////////////////////////////////////////////////////////////////
CWChar::CWChar(const char* pLabel,
			   char c,
			   const CTextDisplayInfo* pTDI)
...
///////////////////////////////////////////////////////////////////////////////////////
CWMarker::CWMarker(const char* pLabel, LPCTSTR pStr)
...
CWBool::CWBool(const char* pLabel, BOOL bValue)
...
CWGroup::CWGroup(const char* pLabel)
...
// append string
void CWString::appendData(CString& s, char bSeperator)
{
	if(m_data.GetLength())
	{
		if(bSeperator=='\n') // actually need \r\n
...
CWInteger::CWInteger	(const char* pLabel, int iValue)
:CWString(pLabel, "", kUserLanguage), m_iValue(iValue)
{
...
CWStatic::CWStatic(const char* pLabel, DWORD dwFlags)
