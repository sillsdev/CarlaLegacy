...
void CToneParseProcess::readParametersFromSFMFile(SFMFile *f)
{
	...
	while(f->getField(sMarker, sField, &bEnabled))
	{
		...
		{
			f->throwParseFailure("ToneParseProcess", sMarker, sField);
		}
	}
}
...
const char* CToneParseProcess::getToolPath()
{
	return "TonePars32";
}
...
{
	CString sBatchFile;
	sBatchFile.Format("%s ToneParse%d",
...
*/
  ...
  if(status.sANAPath.getShortPath().IsEmpty())
	throw(CProcessFailure(this, "Huh?  There was no ANA file path given to the ToneParse process.  You should look into the log of the previous processor (ample or sentrans) and look for errors, then report this as a bug."));
