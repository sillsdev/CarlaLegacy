...
BEGIN_MESSAGE_MAP(CWListView, CView)
	...
	ON_COMMAND(ID_BookmarkOuiNon, OnBookmarkOuiNon)
	ON_COMMAND(ID_Bookmark_PreviousTB, OnBookmarkPrevious)
	ON_COMMAND(ID_Bookmark_NextTB, OnBookmarkNext)
	ON_COMMAND(ID_Bookmark_Clear, OnBookmarkClear)

	ON_UPDATE_COMMAND_UI(ID_Bookmark_NextTB, OnUpdateBookmarkNext)
	ON_UPDATE_COMMAND_UI(ID_Bookmark_PreviousTB, OnUpdateBookmarkPrevious)
	ON_UPDATE_COMMAND_UI(ID_Bookmark_Clear, OnUpdateBookmarkClearAll)
...
void CWListView::OnBookmarkOuiNon()
{
	BOOL bChange=FALSE;
	int nItem = getFirstSelectedRow();
	if( nItem==-1 ) return;


	CWListRowItem *pRowItem = getRowItem(nItem);
	...
	UINT nStateFlag = m_listCtrl.GetItemState( nItem, LVIS_OVERLAYMASK );
	...
	case 0:								// no overlay
		if(lvi.iImage != 99)
		{
			if(pRowItem->m_bEnabled)
				nIndexOverlay=2;		// set overlay #2 - bookmark
			else
				nIndexOverlay=3;		// set overlay #3 - bookmark/disabled
			m_nBookmarkCount++;
			bChange=TRUE;
		}
		break;
	case 256:							// overlay disabled
		if(lvi.iImage != 99)
		{
			if(pRowItem->m_bEnabled)
				nIndexOverlay=2;		// set overlay #2
			else
				nIndexOverlay=3;		// set overlay #3
			m_nBookmarkCount++;
			bChange=TRUE;
		}

		break;
	case 512:							// overlay bookmark
		nIndexOverlay=0;				// remove overlay bookmark
		m_nBookmarkCount--;
		bChange=TRUE;
		break;
	case 768:							// overlay bookmark/disabled
		nIndexOverlay=1;				// set overlay #1 - disabled
		m_nBookmarkCount--;
		bChange=TRUE;
		break;
	...
	if(bChange)
	{
		m_listCtrl.SetItemState( nItem,
								INDEXTOOVERLAYMASK(nIndexOverlay),
								 LVIS_OVERLAYMASK );
		CRect rect;
		m_listCtrl.GetItemRect( getFirstSelectedRow( ), rect, LVIR_BOUNDS );
...
void CWListView::OnBookmarkNext()
{
	BOOL bFound=FALSE;
	int nItem=getFirstSelectedRow();
	int nStart=nItem;
	...
	if(!bFound)
	{
		m_listCtrl.SetItemState (nStart, LVIS_FOCUSED|LVIS_SELECTED, LVIS_SELECTED|LVIS_FOCUSED);
	}
}

void CWListView::OnBookmarkPrevious()
{
	BOOL bFound=FALSE;
	int nItem=getFirstSelectedRow();
	int nStart=nItem;
...
void CWListView::OnUpdateBookmarkNext(CCmdUI* pCmdUI)
{
	pCmdUI->Enable( m_nBookmarkCount );
}

void CWListView::OnUpdateBookmarkPrevious(CCmdUI* pCmdUI)
{
	pCmdUI->Enable( m_nBookmarkCount );
}

void CWListView::OnUpdateBookmarkClearAll(CCmdUI* pCmdUI)
{
	pCmdUI->Enable( m_nBookmarkCount );
}
