...
{
	ASSERTX(m_pLang);
	fout << "\\" << getOpeningSFMMarker() << "\n";
	writeParamsToFile(fout);
	fout << "\\" << getClosingSFMMarker() << "\n";
}
...
void CProcessSequence::readFromFile(SFMFile& f)
{
	...
	while(f.getField(sMarker, sField, &bEnabled))
	{
		if(sMarker == "+Process")
		{
			...
			if(!pTemplate)
			{
				MSG2("CARLA Studio could not find a DLL for the processor with ID: ", sID);
				...
				while(f.getField(sMarker, sField) && sMarker != "-Process");
			}
			else
			{
		...
		{
			MSG2("ProcessSequence Can't understand project file marker: ", sMarker);
			throw(CString("Couldn't read the Process Sequence from the project file."));
		}
	}
}
...
//TRACE("executable at %s\n", _pgmptr);

	WIN32_FIND_DATA findFileData;
	HANDLE hFileInfo = FindFirstFile( "*.DLL", &findFileData);
...
BOOL CProcessSequence::continueProcessing(CProcessStatus * pStatus)
{
	...
	try
	{
		...
		{
			CString s;
			s = "You must first add at least one enabled processor to the ";
			s += getFullName();
			THROWSTRING2(s, "");
		}
					...
					|| m_pProcesses[i]->getInputType() == DICT))			// jdh 3/13/2000
			{
				...
				switch(m_pProcesses[i]->getInputType())
				{
					...
					case INTERLINEAR:	// a cc-table may be the only thing that takes this as input
						...
						break;
					default: throw CProcessFailure(m_pProcesses[i], "This version of CarlaStudio does not handle the input type desired by this processor.");
								break;
				}
			}
		}
...
void CTransferProcessSequence::readFromFile(SFMFile& f)
{
	...
	else throw(CString("Didn't find the \\TargetLangID field."));
}
...
CString  CTransferProcessSequence::getFullName() const
{
	ASSERTX(m_pTargetLang);
	CString s;
	s.Format("%s Transfer To %s Sequence", m_pLang->getName(), m_pTargetLang->getName());
	return s;
}
...
BOOL CTransferProcessSequence::isValidSequence(CString& sInvalidMsg) const
{
	...
	// RULES
	 ...
	 {
		sInvalidMsg = "At least one process must input the source ANA (examples: SENTRANS && STAMP)";
		return FALSE;
	 }
	 ...
	 if(i == m_pProcesses.GetSize())
	 {
		sInvalidMsg = "One proccess must output an ANA file(example: SENTRANS && TRAMPLE)";
		return FALSE;
	 }
	 return TRUE;
}
...
{
	CString s;
	s.Format("%s Analysis Sequence", m_pLang->getName());
	return s;
}
...
BOOL CAnalysisProcessSequence::isValidSequence(CString& sInvalidMsg) const
{
	...
	// RULES
	 ...
	 if(i == m_pProcesses.GetSize())
	 {
		sInvalidMsg = "At least one process must input the source text (example: AMPLE)";
		return FALSE;
	 }
	 ...
	 if(i == m_pProcesses.GetSize())
	 {
		sInvalidMsg = "One proccess must output an ANA file(example: AMPLE)";
		return FALSE;
	 }
...
{
	CString s;
	s.Format("%s Synthesis Sequence", m_pLang->getName());
	return s;
}
...
BOOL CSynthesisProcessSequence::isValidSequence(CString& sInvalidMsg) const
{
	...
	// RULES
	 ...
	 if(i == m_pProcesses.GetSize())
	 {
		sInvalidMsg = "At least one process must input the target ANA (example: STAMP)";
		return FALSE;
	 }
...
{
	CString s;
	s.Format("%s Interlinearizing Sequence", m_pLang->getName());
	return s;
}
...
BOOL CInterlinearProcessSequence::isValidSequence(CString& sInvalidMsg) const
{
	...
	// RULES
	 ...
	 if(i == m_pProcesses.GetSize())
	 {
		sInvalidMsg = "At least one process must input the  ANA (example: INTERGEN)";
		return FALSE;
	 }
...
CProcess* CProcessSequence::getFirstProcessOfType(CString sTypeID)
{
	for(int i=0; i<m_pProcesses.GetSize(); i++)
	{
		if(m_pProcesses[i]->getProcessorID() == sTypeID)
