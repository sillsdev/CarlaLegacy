...
void CProjectDoc::removeLanguage(CLangModelsDoc* pLangDocRemove)
{
	...
	{
		AfxMessageBox("Sorry, you can't remove the only language in the project");
		return;
	}
	...
	CString s;
	s.Format("Do you really want to permanently remove the language %s from this project?", pLangDocRemove->getLang()->getName());
	if(IDYES != AfxMessageBox(s, MB_YESNO|MB_DEFBUTTON2|MB_ICONEXCLAMATION   ))
		return;


#ifndef hab17a1
	if(IDOK != AfxMessageBox("Before removing this language, CarlaStudio will make backups of the project and language files, adding a '.b4r' to their names.  You'll have to delete these backups yourself.\r\nFurther, all open text and ana documents will be closed and you will have to reopen them later.\r\nFinally, Carla Studio will exit and you will have to run it again.", MB_OKCANCEL|MB_DEFBUTTON2|MB_ICONEXCLAMATION   ))
#else // hab17a1
	if(IDOK != AfxMessageBox("Before removing this language, CarlaStudio will make a backups of the project and language files, adding a '.b4r' to their names.  You'll have to delete these backups yourself.\r\nFurther, all open text and ana documents will be closed and you will have to reopen them later.\r\nFinally, Carla Studio will exit and you will have to run it again.", MB_OKCANCEL|MB_DEFBUTTON2|MB_ICONEXCLAMATION   ))
#endif // hab17a1
		return;

	// Make backups

	CString sBackup = CDocument::GetPathName() + ".b4r";

	if(!CopyFile(GetPathName(), sBackup, FALSE))
	{
		AfxMessageBox("Couldn't make a backup of the project file. Language not removed.");
		return;
	}
	...
	for(int i=0; i< m_pLangDocs.GetSize();i++)
	{
		...
		{
			CString sBackup = m_pLangDocs[i]->GetPathName() + ".b4r";
			if(!CopyFile(m_pLangDocs[i]->GetPathName(), sBackup, FALSE))
			{
				AfxMessageBox("Couldn't make a backup of one the languages. Language not removed.");
				return;
			}
		}
	}
