...
public:
	...
	virtual void doUpdate();//CFont* pSrcFont, CFont* pTrgFont, CFont* pDEFAULTFont);
	void appendData(CString& s, char bSeperator= '\n');
	~CWString();
//	CWString() {};

	void setData(CString& sStr) {m_data = sStr; }
	void setData(const char* lpszStr) {ASSERTX(lpszStr); m_data = lpszStr; }
...
{
public:
	CWInteger(const char* pLabel, int iValue);
	virtual void retrieveData();
...
{
public:
	CWMarker(const char* pLabel, LPCTSTR pStr=NULL);
protected:
...
public:
	~CWStatic();
	CWStatic(const char* pLabel, DWORD dwFlags=NULL);
	int AddToView(int left,int top,  int labelWidth, CWnd* parent, UINT id);
	virtual void retrieveData(){};
...
public:
	BOOL isTrue() const;
	~CWBool();
	CWBool(const char* pLabel, BOOL bValue = FALSE);
...
public:
	...
	CWRadio(int iWhichIsOn,
			const char* pLabel1,
			const char* pLabel2,
			const char* pLabel3=0,
			const char* pLabel4=0);
