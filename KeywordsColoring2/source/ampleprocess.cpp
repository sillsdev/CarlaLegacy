...
const char* CAmpleProcess::getToolPath()
{
	return "AMPLE32";
}
...
CString CAmpleProcess::getCommandLine(CProcessStatus& status)
{
	...
	sout << "-i " << INPUTFILE.getQuotedPath(); //getFileFullName(); // leave off the dir
	sout << " -f " << m_sCMDPath.getQuotedPath(); //getShortPath();
	sout << " -o " << m_sANAPath.getQuotedPath() << " ";
...
void CAmpleProcess::processRAWTextFile(CProcessStatus& status)
{
	try
	{
		...
		{		throw(CProcessFailure(this, "Failed to parse the source file"));
		}
...
void CAmpleProcess::outputSupportFiles(CProcessStatus& status)
{
	...
	if (status.m_pProcPrefs->getDoInterlinearize())
	  fout <<   pSourceMFS->getCtrlFilePath("itcd.tab") << "\n";
	else
