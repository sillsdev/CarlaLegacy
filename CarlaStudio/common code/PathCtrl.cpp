...
void CPathCtrl::init()
{
	...
	m_pButton->Create("x", WS_CHILD|WS_VISIBLE, r, this, kButtonID);
	...
	m_pStatic->Create("x",  SS_SUNKEN|WS_CHILD|WS_VISIBLE, r, this);
	updatePathDisplay();
...
void CPathCtrl::OnAddButton()
{
	...
	if(m_nFlags & kSelectFolder)
	{
		...
		CDirDialog dlg;
		dlg.m_strWindowTitle = "Select Folder";
		...
		{
			m_path = dlg.m_strPath +"\\"; //dlg.GetPathName() +"\\";
		}
