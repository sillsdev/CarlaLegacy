...
m_dwColumnEditBitfield(~0)	// all editabled by default
{
	...
	m_pStrBuff = new char[kBuffSize+1];	// 20480 = 20k
...
{
TRACE("CEditListCtrl::OnEndLabelEdit\n");
...
{
//TRACE("CEditListCtrl::OnLButtonDown\n");
	int index;
	m_bDidDrag=FALSE;
...
void CEditListCtrl::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	//	sSubItem = GetItemText(nItem, nColumn);
		...
		//char* buff = sSubItem.GetBuffer(m_nMaxItemLength+1);
	...
	// to make sure noone exceeds this maximum length.
		...
		{
			CString s;
			s.Format("Please report this problem: The text for %s was %d characters long, but the maximum allowed is only %d characters.\n",
...
/*	else
	{
		int colnum;
		int index;
		TRACE("Up editing=%d last=%d\n", m_bInEditingMode, m_nLastClickedItem);
		if(!m_bInEditingMode && ( index = HitTestEx( point, &colnum )) != -1 )
		{
			TRACE("    index=%d column=%d", index, colnum);
			UINT flag = LVIS_FOCUSED;
			if(  m_nLastClickedItem == index &&
				(GetItemState( index, flag ) & flag) == flag
				&& colnum >= 0)
...
void CEditListCtrl::finishDrag(int iFrom, int iTo)
{
	...
	if(lvi.pszText != LPSTR_TEXTCALLBACK)
	{
		...
		{
			strcpy(lvi.pszText, (LPCTSTR)(GetItemText(iFrom,col)));
			lvi.iSubItem = col;
			SetItem(&lvi);
		}
	}
...
CFont* CEditListCtrl::getFontForSubItem(int nItem, int nCol)
{
	if(m_dwFlags & MULTIPLE_FONTS)	// in this case user must sub
		AfxMessageBox("Since subclass of CEditListCtrl has set the flag MULTIPLE_FONTS, it must override getFontForSubItem()");
...
{
	return "Subclass of CEditListCtrl must override getItemText(LPARAM)";
}
...
// from zafir's article "Sorting the list based on text in any column" in www.codeguru.com
...
void CGuruColumn::insertColumnIntoCtrl(CListCtrl *pLC)
{
	...
	lv.fmt = LVCFMT_LEFT;
	lv.cx = m_nWidth;
	lv.pszText = (char*)LPCTSTR(m_sName);
...
{
	TRACE("Subclass of CEditListCtrl must override activateItem if you want it to do something");
}
...
{
	TRACE("Subclass of CEditListCtrl must override deleteItem if you want it to do something");
}
...
{
	AfxMessageBox("Subclass of CEditListCtrl must override addNewItem since DOUBLE_CLICK_NEW is specified");
}
...
{
	AfxMessageBox("Since subclass of CEditListCtrl has set the flag MULTIPLE_FONTS, it must override getLargestFontHeight()");
	return 12;
}
