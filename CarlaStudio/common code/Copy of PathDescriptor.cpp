...
void CPathDescriptor::parsePath()
{
	...
	{	// TO DO: How to check for a valid-looking path?
		_splitpath( LPCTSTR(*this), drive, dir, fname, ext );
...
//	HANDLE h = FindFirstFile(*this, &fileInfo);
	...
	return(0 == _access(*this, 0));
}

BOOL CPathDescriptor::fileExistsAndIsClosed() const
{
	if(0 != _access(*this, 06))//read & write permission
		return FALSE;
...
{
   if(!lpszPathName || !strlen(lpszPathName))
	   return CString("");
...
DWORD checkForFileError(LPCTSTR lpszPath1, LPCTSTR lpszPath2, BOOL bIgnoreERROR_ALREADY_EXISTS)
{
...
BOOL nukeDirectory(LPCSTR lpszDirPath)
{
	...
	if(bOK)
	{
		bOK = RemoveDirectory(lpszDirPath);
	}
	return bOK;
}
...
CTime CPathDescriptor::getModifiedTime() const
{
	...
	{
		checkForFileError(getFullPath());	// should be pretty rare
		return CTime();
	}
...
BOOL CPathDescriptor::moveTo(LPCTSTR lspzToPath, BOOL bFailIfExists)
{
