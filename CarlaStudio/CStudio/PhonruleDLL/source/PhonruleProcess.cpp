...
void CPhonruleProcess::readParametersFromSFMFile(SFMFile *f)
{
	...
	while(f->getField(sMarker, sField, &bEnabled))
	{
		if(sMarker=="-Process")
			...
			m_cCommentChar = sField[0];
		else
		{
			f->throwParseFailure("PhonruleProcess", sMarker, sField);
		}
	}
}
	...
	try
	{
		...
		switch(status.m_iCurrentSeqFunction)
		{
			...
			default: // the user should never be able to make this happen
				throw CProcessFailure(this, "Phonrule should only be called as part of analysis or synthesis");
		}

		// combine all runs into one log
		sLOGPath = status.makeTempPath(pMFS->getAbrev()+"-phrl", ".log");
		sLOGPath.deleteFile();

		for(int iRootFileIndex = 0; iRootFileIndex< (*pRootUnifiedDicts).GetSize(); iRootFileIndex++)
		{

			CString sBaseName = (*pRootUnifiedDicts)[iRootFileIndex]->getFileName() + "-phrl";
			CPathDescriptor sOutputDictPath = status.makeTempPath(sBaseName,".dic");
			...
			BOOL OK =  setupProcess(lpszErrorStr,
					...
					pMFS->getPhonrulePath(),			// lpszRulesPath
					(*pRootUnifiedDicts)[iRootFileIndex]->getFullPath(),	// lpszDictInPath
					sOutputDictPath.getFullPath(),			// lpszDictOutPath
					sLOGPath.getFullPath(),				// lpszLogFilePath
					TRUE); // append log
			...
			if(INVALID_HANDLE_VALUE == FindFirstFile(sOutputDictPath.getFullPath(), &fileInfo))
				throw(CProcessFailure(this, "Phonrule DLL did not produce the expected dictionary.  Check the phonrule log for more information."));
		...
		CResultStreamFile* logStream =
			new CResultStreamFile(
					new CResultStreamDescriptor(this,
												"Phonrule Log", //short description
												"Log of this process; read it for status and diagnostics.", // long description
												pMFS->getAbrev()+" Phnrl Log"
											),
