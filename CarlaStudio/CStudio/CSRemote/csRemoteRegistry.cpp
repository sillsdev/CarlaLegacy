...
LONG getRegistryKeyToCStudioParams(HKEY *paramsKey)
{
	*paramsKey = NULL;
	...
	CHECKFAIL(softwareKey.Open(HKEY_CURRENT_USER, "SOFTWARE", KEY_READ));
	CHECKFAIL(silKey.Create(softwareKey, "SIL"));
	CHECKFAIL(cstudioKey.Create(silKey, "CarlaStudio"));
	CHECKFAIL(tempParamsKey.Create(cstudioKey, "RemoteParams"));
