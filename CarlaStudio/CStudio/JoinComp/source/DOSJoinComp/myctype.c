...
#else
/*
 *  add the standard (English) pairs on the first call
 */
if (pTextCtl_io->pLowercaseLetters == NULL)
	...
	for ( c = 'z' ; c >= 'a' ; --c )
	{
	pLower = allocMemory(sizeof(LowerLetter));
	szBuffer[0] = c;
	pLower->pszLower = (unsigned char *)duplicateString(szBuffer);
	pLower->pNext    = pTextCtl_io->pLowercaseLetters;
	pTextCtl_io->pLowercaseLetters = pLower;

	pUpper = allocMemory(sizeof(UpperLetter));
	szBuffer[0] = toupper(c);
	pUpper->pszUpper = (unsigned char *)duplicateString(szBuffer);
	pUpper->pNext    = pTextCtl_io->pUppercaseLetters;
	pTextCtl_io->pUppercaseLetters = pUpper;

	pLower->pUpperList            = allocMemory(sizeof(StringList));
	pLower->pUpperList->pszString = (char *)pUpper->pszUpper;
	pLower->pUpperList->pNext     = NULL;

	pUpper->pLowerList            = allocMemory(sizeof(StringList));
	pUpper->pLowerList->pszString = (char *)pLower->pszLower;
	pUpper->pLowerList->pNext     = NULL;
	}
	}
#endif
/*
