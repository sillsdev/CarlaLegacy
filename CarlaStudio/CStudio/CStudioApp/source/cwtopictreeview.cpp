...
//	char* lpsz;
	...
	//	pCurrentMainTopic->addToTree(subItem);
		...
		if (pCurrentMainTopic->IsKindOf(RUNTIME_CLASS(CWHeadingTopic)))
		{
			...
			subItem.item.pszText = (char*) LPCTSTR( pHeadingTopic->m_label); // this is ok so long as we don't change the label
			...
			//	subItem.hParent = hHeadingItem;
				...
				subItem.item.pszText = (char*) LPCTSTR( pSubTopic->m_label); // this is ok so long as we don't change the label
		...
		{
			//CWHeadingTopic* pHeadingTopic = (CWHeadingTopic*)pCurrentMainTopic;
			subItem.item.pszText = (char*) LPCTSTR( pCurrentMainTopic->m_label); // this is ok so long as we don't change the label
