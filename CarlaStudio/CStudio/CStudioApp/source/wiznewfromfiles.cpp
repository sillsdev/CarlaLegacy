...
BOOL CPageNewFromFilesFinish::OnSetActive()
{
	...
	m_sFolderPath.Format("%s Control Files", pWiz->m_langDisplayPage.m_sAbrev);

	/*	ostrstream sout;
	CWizNewFromFiles* pWiz = (CWizNewFromFiles*)getOwner();
	addLine(sout, "Phonrule", pWiz->m_analysisPage.m_sPhonrulePath);
	addLine(sout, "Dict Code Table", pWiz->m_analysisPage.m_sCodeTablePath);
	addLine(sout, "TextIn", pWiz->m_analysisPage.m_sTextInPath);
	addLine(sout, "Ortho Changes", pWiz->m_analysisPage.m_sDictOrthoPath);
	addLine(sout, "Analysis Data Ctrl", pWiz->m_analysisPage.m_sAnalysisPath);
	addLine(sout, "SENTRANS Disambig Rules", pWiz->m_analysisPage.m_sSTDisambigPath);
	addLine(sout, "SENTRANS Glossing Rules", pWiz->m_transferPage.m_sSTGlossingPath);
	addLine(sout, "STAMP Synthesis", pWiz->m_synthPage.m_sStampSynth);
	addLine(sout, "STAMP Text-Out Control", pWiz->m_synthPage.m_sTextOut);
...
{
	handleFileButton(m_sAnalysisPath, "ctl", "Analysis Control|*ad.ctl;*ad01.ctl|All Files|*.*||");
}

void CPageAnalysisFiles::OnBUTTONTextIn()
{
	handleFileButton(m_sTextInPath, "ctl", "TextIn (*intx.ctl)|*intx.ctl|All Files|*.*||" );
}

void CPageAnalysisFiles::OnBUTTONDictOrtho()
{
	handleFileButton(m_sDictOrthoPath, "tab", "Source Ortho Change Table (*ordc.tab)|*ordc.tab|All Files (*.*)|*.*||");
}

void CPageAnalysisFiles::OnBUTTONPhonrule()
{
	handleFileButton(m_sPhonrulePath, "rul", "Phonrule Table (*.rul)|*.rul|All Files (*.*)|*.*||");
}

void CPageAnalysisFiles::OnBUTTONCodeTable()
{
	handleFileButton(m_sCodeTablePath, "tab", "Code Table (*cd.tab)|*cd.tab|All Files (*.*)|*.*||");
}

#ifndef hab17a1
void CPageAnalysisFiles::OnBUTTONCodeTableIt()
{
	handleFileButton(m_sCodeTableItPath, "tab", "Code Table (*cd.tab)|*cd.tab|All Files (*.*)|*.*||");
}
#endif // hab17a1

void CPageAnalysisFiles::OnBUTTONSTDisambig()
{
	handleFileButton(m_sSTDisambigPath, "sen", "SENTRANS (*.sen)|*.sen|All Files (*.*)|*.*||" );
}
...
{
	handleFileButton(m_sRoot1Path, "dic", "Root Dictionary (*.dic; *.db)|*.dic; *.db|All Files (*.*)|*.*||");
}

void CPageDictionaries::OnBTNRoot2()
{
	handleFileButton(m_sRoot2Path, "dic", "Root Dictionary (*.dic; *.db)|*.dic; *.db|All Files (*.*)|*.*||");
}

void CPageDictionaries::OnBTNRoot3()
{
	handleFileButton(m_sRoot3Path, "dic", "Root Dictionary (*.dic; *.db)|*.dic; *.db|All Files (*.*)|*.*||");
}

void CPageDictionaries::OnBTNRoot4()
{
	handleFileButton(m_sRoot4Path, "dic", "Root Dictionary (*.dic; *.db)|*.dic; *.db|All Files (*.*)|*.*||");
}

void CPageDictionaries::OnBTNRoot5()
{
	handleFileButton(m_sRoot5Path, "dic", "Root Dictionary (*.dic; *.db)|*.dic; *.db|All Files (*.*)|*.*||");
}
*/
...
{
	handleFileButton(m_sSTGlossingPath, "sen", "SENTRANS Glossing Control File (*.sen)|*.sen|All Files (*.*)|*.*||");
}
...
{
	handleFileButton(m_sStampSynth, "chg", "Stamp Synthesis Control File (*.chg)|*.chg|All Files (*.*)|*.*||");
}

void CPageFilesSynth::OnbtnTextOut()
{
	handleFileButton(m_sTextOut, "ctl", "Text Out Control File (*.ctl)|*.ctl|All Files (*.*)|*.*||");
}

#ifndef hab17a1
void CPageFilesSynth::OnbtnCodeTableStamp()
{
	handleFileButton(m_sCodeTableStampPath, "tab", "Code Table (*cd.tab)|*cd.tab|All Files (*.*)|*.*||");
}
#endif // hab17a1
...
{
	handleFileButton(m_sCmdPath, "cmd", "Ample CMD File(*.cmd)|*.cmd|All Files (*.*)|*.*||");
	...
	// parse the cmd file
	ifstream fin(m_sCmdPath, ios::in | ios::nocreate);
	char lpsz[MAX_PATH];
	fin.getline(lpsz, MAX_PATH);
	m_sAnalysisPath = getFilePath(sDir, lpsz);

	fin.getline(lpsz, MAX_PATH);
	m_sCodeTablePath = getFilePath(sDir, lpsz);

	fin.getline(lpsz, MAX_PATH);
	if(strlen(lpsz))
		m_sDictOrthoPath = getFilePath(sDir, lpsz);
