 ...
 */
BOOL WinExecWait(const char *szCmdLine, UINT nCmdShow)
{
  ...
  si.cbReserved2 = 0;
  si.lpReserved2 = 0;
  if (!CreateProcess (NULL, (char*)szCmdLine, NULL, NULL, FALSE,
	  CREATE_DEFAULT_ERROR_MODE, NULL, NULL, &si, &pi))
  {
	MessageBox (NULL, "WinExecWait failed.", "Installation Program", MB_OK);
	return FALSE;
  }
...
*/
	...
	if(!path1->fileExistsAndIsClosed())
	{
		CString s("Waiting for ");
		s+= getDisplayName();
		s+= " to finish.  Click 'Cancel' to abort processing.";
		pStatus->progress(s);
	}
...
CString CProcess::getFileTypeDisplayName(PROCESS_FILE_TYPE t)
{
	switch(t)
	{
		case INTERLINEAR: return "Interlinear Text"; break;
		case INFORMATION: return "Information"; break;
		case DICT: return "Dictionary"; break;
		case RAWTEXT: return "Text"; break;
		case ANA: return "ANA"; break;
		case PTEXT: return "PText"; break;
