...
void CIntergenMarkersCtrl::makeColumns()
{
	...
	insertColumn("Description", 400, NULL);
	insertColumn("SF Marker", 100, CGuruColumn::ALLOW_EDITTING);// use the list horiz scroll bar to show a long comment
...
void CIntergenMarkersCtrl::addItem(CIntergenMarker *pMarker)
{
	...
	lvi.pszText = (char*) LPSTR_TEXTCALLBACK;
...
void CIntergenMarkersCtrl::setItemText(int nItem, int nSubItem, LPCTSTR pszText)
{
	...
	if(nSubItem==0 && pszText && strlen(pszText)>0)
	{
		CString s = pszText;
		s.TrimLeft();
