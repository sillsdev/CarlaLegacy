...
// but we must cast to non-const since there isn't a const char* type

CParseStream::CParseStream(const char* pszBuffer, char cCommentChar) //, BOOL bOwnTheBuffer)
:  m_buff((char*)pszBuffer), m_cCommentChar(cCommentChar), m_index((char*)pszBuffer) //,m_bOwnTheBuffer(bOwnTheBuffer)
{
	ASSERTX(m_buff);
}
...
BOOL CParseStream::swallow(const char* sMatch)
{
	int len = strlen(sMatch);

	eatWhiteSpace();
	if(0==strncmp(sMatch, m_index,  len) )
	{	m_index += len;
		return TRUE;
	}
	else
...
BOOL CParseStream::swallow(char cMatch)
{
	eatWhiteSpace();
	if(m_index[0] == cMatch)
	{	++m_index;
...
BOOL CParseStream::upTo(CString& sResult, char cUpTo1, char cUpTo2, char cUpTo3, char cUpTo4, LPCTSTR lpszUpTo5)
{
	...
	int iLen = 0;
	if(lpszUpTo5)
		iLen = strlen(lpszUpTo5);

	while( *m_index && *m_index != '\\'
			&& *m_index != cUpTo1 && *m_index != cUpTo2
			&& *m_index != cUpTo3 && *m_index != cUpTo4
			&& (lpszUpTo5?(strnicmp(m_index,lpszUpTo5, iLen) !=0 ):TRUE)
		)
		++m_index;
...
BOOL CParseStream::getChar(char &c)
{
	c = *m_index;
	++m_index;
	return c != '\0';
