	...
	else if(pn->code == EN_MSGFILTER)
	{
		...
		if(pm->msg == WM_KEYDOWN)
		{
			...
			if(((CInputDoc*)(m_pOwner->GetDocument()))->m_bBaseIsEditable) 		// jdh 12April2001 can't  modify src view if readonly
				...
				AfxMessageBox("This document was opened as \"Read-only\", meaning you can't make changes to it.  If you want to make changes, you'll need to close this document, make sure the Read-only flag of the file is cleared, and then open it again.");
		}
	}
