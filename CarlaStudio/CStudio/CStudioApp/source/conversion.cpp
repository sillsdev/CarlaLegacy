...
//	ASSERTX(*lpszDirOfCMenuLang);
	...
	while(f.getField(sMarker, sField, &bEnabled))
	{
		...
		{}	// skip it
			...
			TRACE("****UNKNOWN FIELD in %s.set [%s]  [%s]\n", m_sAbrev, sMarker, sField);
	}
	return TRUE;
}
...
void CCarlaLanguage::convertFromCarla()
{
	...
	if(m_mfs.m_sAdctlFileDirectory == m_sTempConvertFromDirectory)
		m_mfs.m_sAdctlFileDirectory = "";	// make empty if not special
...
//	else
	...
	m_mfs.m_sAdctlFileDirectory = "";
	m_mfs.m_sAdctlFileName = "";

	// ASSUMPTION: the rest of the control files are always in the same dir as the <abrev>carla.set file
	copyCtrlFile(m_sTempConvertFromDirectory, "intx.ctl");
	copyCtrlFile(m_sTempConvertFromDirectory, "outtx.ctl");
#ifndef hab17a1
	copyCtrlFile(m_sTempConvertFromDirectory, "cd.tab");
				// also create interlinear and synthesis copies
	copyCtrlFile(m_sTempConvertFromDirectory, "cd.tab", "itcd.tab");
	copyCtrlFile(m_sTempConvertFromDirectory, "cd.tab", "sycd.tab");
#else // hab17a1
	copyCtrlFile(m_sTempConvertFromDirectory, "cd.tab");
#endif // hab17a1
	copyCtrlFile(m_sTempConvertFromDirectory, "phon.rul");
	copyCtrlFile(m_sTempConvertFromDirectory, "ordc.tab");
	copyCtrlFile(m_sTempConvertFromDirectory, "disam.sen");
	copyCtrlFile(m_sTempConvertFromDirectory, "adj.sen");
	copyCtrlFile(m_sTempConvertFromDirectory, "synt.chg");
...
*/
	m_sTempConvertFromDirectory = "";	// clear this so we're not called to convert again
...
void CCarlaLanguage::convertFromFiles()
{
	...
	m_mfs.m_sAdctlFileDirectory = ""; // empty string means same as other control files

	CopyFile(m_pFilesWiz->m_analysisPage.m_sAnalysisPath, m_mfs.getCtrlFilePath("ad.ctl"), FALSE);
	CopyFile(m_pFilesWiz->m_analysisPage.m_sTextInPath, m_mfs.getCtrlFilePath("intx.ctl"), FALSE);
	CopyFile(m_pFilesWiz->m_analysisPage.m_sCodeTablePath, m_mfs.getCtrlFilePath("cd.tab"), FALSE);
#ifndef hab17a1
	...
	if(m_pFilesWiz->m_analysisPage.m_sCodeTableItPath.GetLength())
	  CopyFile(m_pFilesWiz->m_analysisPage.m_sCodeTableItPath,
		   m_mfs.getCtrlFilePath("itcd.tab"), FALSE);
	else
	...
	if(m_pFilesWiz->m_synthPage.m_sCodeTableStampPath.GetLength())
	  CopyFile(m_pFilesWiz->m_synthPage.m_sCodeTableStampPath,
		   m_mfs.getCtrlFilePath("sycd.tab"), FALSE);
	else
	  CopyFile(m_pFilesWiz->m_analysisPage.m_sCodeTablePath,
		   m_mfs.getCtrlFilePath("sycd.tab"), FALSE);
#endif // hab17a1
	if(m_pFilesWiz->m_analysisPage.m_sPhonrulePath.GetLength())
	   CopyFile(m_pFilesWiz->m_analysisPage.m_sPhonrulePath, m_mfs.getCtrlFilePath("phon.rul"), FALSE);
	if(m_pFilesWiz->m_analysisPage.m_sDictOrthoPath.GetLength())
		CopyFile(m_pFilesWiz->m_analysisPage.m_sDictOrthoPath, m_mfs.getCtrlFilePath("ordc.tab"), FALSE);

	// copy non-transfer sentrans-related stuff
	if(m_pFilesWiz->m_analysisPage.m_sSTDisambigPath.GetLength())
		CopyFile(m_pFilesWiz->m_analysisPage.m_sSTDisambigPath, m_mfs.getCtrlFilePath("disam.sen"), FALSE);
	if(m_pFilesWiz->m_transferPage.m_sSTGlossingPath.GetLength())
		CopyFile(m_pFilesWiz->m_transferPage.m_sSTGlossingPath, m_mfs.getCtrlFilePath("adj.sen"), FALSE);
	...
	if(m_pFilesWiz->m_synthPage.m_sTextOut.GetLength())
		CopyFile(m_pFilesWiz->m_synthPage.m_sTextOut, m_mfs.getCtrlFilePath("outtx.ctl"), FALSE);
	...
	delete m_pFilesWiz;
	m_pFilesWiz = 0;
	m_sTempConvertFromDirectory = "";	// this should already be empty, since it's only for CARLAMENU conversion
...
void CCarlaLanguage::copyFile(CString& sFromDir, CString& sToDir, CString& sFileName, LPCTSTR lpszToFileName)
{
	...
	{
		LOG4("Couldn't copy the file: ", sFromPath, " to ", sToPath + "\r\n");
	}
}
