...
class CWList : public CObject
{
	public:
...
#ifndef hab211
		void write(ostream& fout, char cCommentChar, char *pszCode) const;
#else //hab211
		...
		virtual CString getSecondaryEntryTypeDisplayName() const {return "Error";}
	...
	protected:
		...
		void insertColumn(CListCtrl &clc, char* pszName, int iWidth, int nIconIndex=-1);
...
class CWTestList : public CWList
{
	public:
		...
		CWTestList(LPCTSTR lpszSFMCode, 	const CTextDisplayInfo* pTDI)
			...
			m_sSFMCode(lpszSFMCode) {};
		virtual void insertColumns(CListCtrl &clc);
		virtual const CString getDisplayName() const {static CString sName("Test"); return sName;}
		CString getSFMCode() const {return m_sSFMCode;}
...
class CWAllomorphSelectionList : public CWList
{
	public:
		CWAllomorphSelectionList(const CTextDisplayInfo* pTDI)
			: CWList(4,0, pTDI) {};
		virtual void insertColumns(CListCtrl &clc);
		virtual const CString getDisplayName() const {static CString sName("AllomorphSelection"); return sName;}
		virtual BOOL getUseLangFontInColumn(int iColumn) const;
