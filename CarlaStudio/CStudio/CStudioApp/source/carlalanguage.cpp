...
BOOL CCarlaLanguage::save(LPCTSTR lpszProjectDirectory)
{
	...
	if(!fout.is_open())
	{	CString s("Couldn't save the language file: ");
		s += getFilePath();
	...
	if(m_pFilesWiz)	// added mar 11 99 for beta 7
	{
		if(!m_pFilesWiz->m_analysisPage.m_sCommentChar.IsEmpty())
			m_commonModel.m_cCommentChar = m_pFilesWiz->m_analysisPage.m_sCommentChar.GetAt(0);
	}
	fout << "\\commentChar " << m_commonModel.m_cCommentChar << "\n"; // added feb 99

	#define WRITEPREFS(M,LIST) fout << "\\"<<M<<" "<< LIST.getPrefsField()<<'\n';
	...
	{
		LOG("--------BEGIN CARLMENU CONVERSION FOR ");
		LOG(getName());
		LOG ("\r\n");
		LOG ("Copying control files...\r\n");
		convertFromCarla();	// first time, just copy over the files
		LOG ("Loading control files...\r\n");
		loadControlFiles(TRUE, FALSE);
		LOG("--------FINISHED CARLMENU CONVERSION FOR ");
		LOG(getName());
		LOG ("\r\n");
	}
	else if(m_pFilesWiz)
	{
		LOG("--------BEGIN FROM-FILES CREATION\r\n");
		convertFromFiles();
		loadControlFiles(FALSE, TRUE);

		LOG("--------FINISHED FROM-FILES CREATION\r\n");
	}
...
BOOL CCarlaLanguage::open(LPCTSTR lpszPathName)
{
	...
	CSimpleProgressBar pb("");
	pb.SetTextEx("Loading", getName(), " Lang File...");

	SFMFile *pFile = new SFMFile(sPath, getCommentChar());
	if(!pFile->ensureOpen("Language Document"))
		return FALSE;
	m_mfs.m_sCtrlFilesDirectory = getDirectory(sPath);

	// rename?
	if(::_stricmp(m_mfs.m_sCtrlFilesDirectory.Right(14), "Control Files\\"))
	{
		int len = m_mfs.m_sCtrlFilesDirectory.GetLength();
		CString sOldDirName = m_mfs.m_sCtrlFilesDirectory.Left(len-1);
		CString sNewDirName = m_mfs.m_sCtrlFilesDirectory.Left(1+len-sizeof("Files\\")) + CONTROL_FOLDER_SUFFIX;

		if(-1 == _access(sNewDirName, 00))	// directory not found
		{
			CString s;
			s.Format("This version of CarlaStudio uses a new name for the folder that contains control files.\n\nThe folder named %s will now be renamed to %s",
						sOldDirName, sNewDirName);
			AfxMessageBox(s, MB_ICONINFORMATION   );
			if(0 == _access(m_mfs.m_sCtrlFilesDirectory, 00))	// is the dir of the old name there?
			{
				// rename it
				TRY
				{
			...
			m_mfs.m_sCtrlFilesDirectory = sNewDirName + "\\";
		}
	}
	...
	while(pFile->getField(sMarker, sField, &bEnabled))
	{
		...
		else if(sMarker == "name")
			m_sName = sField;
		else if(sMarker == "abrev")
			setAbrev(sField);
		...
		else if(sMarker ==  "commentChar") // added feb 99
		{
			if(!sField.IsEmpty())
				m_commonModel.m_cCommentChar.setData(sField[0]);
		}
		else if (sMarker == "interface")
		{
			if(sField == "carlaMenuStyle")
...
#define CHECKPREFS(M,LIST) else if(sMarker==M) LIST.readPrefsField(sField)
		...
		{}	// skip it
		else if (sMarker == "id")	//comment
		{}
		else if (sMarker == "co")	//comment
		{
			// comments in here are carlastudio comments, so not reason to tell the user
		}
		else if (sMarker == "CopyItxToSrcDir")	//this has been moved to the processing prefs
		{}
		//else checkAndReadBool("CopyItxToSrcDir", m_bCopyInterlinearToSrcDir)
		//else checkAndReadBool("LeaveNonGoalFilesInTemp", m_bLeaveNonGoalsInTempDir)

		else if (sMarker == "DisplayFlags") // jdh 11/9/99
		{
			m_wDisplayFlags = atol(sField);
		}
		else
...
CString CCarlaLanguage::getLanguageDirectory(LPCTSTR lpszProjectDirectory,  BOOL bOKToCreate)
{
	...
	_splitpath( lpszProjectDirectory, drive, dir, fname, ext );
...
BOOL CCarlaLanguage::loadControlFiles(BOOL bFirstLoadAfterCarlaMenuImport, BOOL bFirstLoadAfterFilesImport)
{
	...
	CString sPath = m_mfs.getCtrlFilePath("itcd.tab");
	if (!fTestForExistence.Open(sPath, CFile::modeReadWrite))
				// It's not there. Use cd.tab
	  CopyFile(m_mfs.getCtrlFilePath("cd.tab"), sPath, FALSE);
	else
	...
	m_codeTable.loadFromFile(m_mfs.getCtrlFilePath("cd.tab"), getCommentChar());
#endif // hab17a1
	progress.SetText("Loading phonrule...");
	m_phonruleModel.loadFromFile(m_mfs.getCtrlFilePath("phon.rul"), m_commonModel, bDoLogOptionalSrcFiles);

	progress.SetText("Loading TextIn control file...");
	m_textInModel.loadFromFile(m_mfs.getCtrlFilePath("intx.ctl"), m_commonModel, bDoLogOptionalTarFiles);

	progress.SetText("Loading TextOut control file...");
	m_textOutModel.loadFromFile(m_mfs.getCtrlFilePath("outtx.ctl"), m_commonModel, bDoLogOptionalSrcFiles);

	progress.SetText("Loading DictOrtho control file...");
	m_dictOrthoModel.loadFromFile(m_mfs.getCtrlFilePath("ordc.tab"), m_commonModel, bDoLogOptionalSrcFiles);

	progress.SetText("Loading Analysis Data Control File...");
	m_analysisModel.loadFromFile(m_mfs.getADCtlPath(), m_commonModel);

	progress.SetText("Loading Sentrans Disambig File...");
	m_sentransDisambigModel.loadFromFile(m_mfs.getCtrlFilePath("disam.sen"), m_commonModel, bDoLogOptionalTarFiles);

	progress.SetText("Loading Sentrans Gloss Adjustment File...");
	m_sentransGlossAdjustmentModel.loadFromFile(m_mfs.getCtrlFilePath("adj.sen"), m_commonModel, bDoLogOptionalTarFiles);

	// JDH 6/2/99 Added Sentrans Adjust Rules Set for analysis
	progress.SetText("Loading Sentrans Anan Adjustment File...");
	BOOL bFound = m_sentransAnaAdjustmentModel.loadFromFile(m_mfs.getCtrlFilePath("ana.sen"), m_commonModel, FALSE);
	if(!bFound)
	{
		CString s("Few Carla users will need these rules.  They allow you to use Sentrans lexical changes, not for glossing or transfer, but for analysis of the source language.  In order to use any rules you do put here, you will need to add a Sentrans Adjust Analysis procesor to your Analysis Sequence for this language.");
		CListEntryComment *com = new CListEntryComment(s);
		m_sentransAnaAdjustmentModel.m_rules.addItem(com);
	}

	progress.SetText("Loading Stamp Synthesis File...");
...
{
	if(!m_phonruleModel.writeFile(m_mfs.getCtrlFilePath("phon.rul"), m_commonModel, m_codeTable, m_mfs.getUsingUnifiedDicts()))
		return FALSE;
	...
	if(!m_codeTable.writeFile(m_mfs.getCtrlFilePath("cd.tab"),
		...
		return FALSE;
	if(!m_interCodeTable.writeFile(m_mfs.getCtrlFilePath("itcd.tab"),
							m_analysisModel.maxPrefixes>0,
							m_analysisModel.maxInfixes>0,
							m_analysisModel.maxSuffixes>0,
													m_mfs.getUsingUnifiedDicts()))
		return FALSE;
	if(!m_synthCodeTable.writeFile(m_mfs.getCtrlFilePath("sycd.tab"),
							m_analysisModel.maxPrefixes>0,
							m_analysisModel.maxInfixes>0,
							m_analysisModel.maxSuffixes>0,
													m_mfs.getUsingUnifiedDicts()))
	...
	if(!m_textInModel.writeFile(m_mfs.getCtrlFilePath("intx.ctl"), m_commonModel))
		return FALSE;

	// jdh 9/1/99 added FALSE for "include scl" parameter
	if(!m_textOutModel.writeFile(m_mfs.getCtrlFilePath("outtx.ctl"), m_commonModel, FALSE))
		return FALSE;

	// jdh 9/1/99 add this special intergen-only version of outtxt.ctl
	if(!m_textOutModel.writeFile(m_mfs.getCtrlFilePath("outit.ctl"), m_commonModel, TRUE))
		return FALSE;

	if(!m_dictOrthoModel.writeFile(m_mfs.getCtrlFilePath("ordc.tab"), m_commonModel))
		return FALSE;

	if(!m_analysisModel.writeFile(m_mfs.getADCtlPath(), m_commonModel))
		return FALSE;

	if(!m_sentransDisambigModel.writeFile(m_mfs.getCtrlFilePath("disam.sen"), m_commonModel))
		return FALSE;

	if(!m_sentransGlossAdjustmentModel.writeFile(m_mfs.getCtrlFilePath("adj.sen"), m_commonModel))
		return FALSE;

	if(!m_sentransAnaAdjustmentModel.writeFile(m_mfs.getCtrlFilePath("ana.sen"), m_commonModel)) //jdh 6/2/99
		return FALSE;
	...
	if(!m_stampSynthesisModel.writeFile(m_mfs.getCtrlFilePath("synt.chg"), m_commonModel))
		return FALSE;
...
void CCarlaLanguage::setAbrev(LPCSTR lpszAbrev)
{
	ASSERTX(lpszAbrev);
	ASSERTX(strlen(lpszAbrev));
	m_sAbrev = lpszAbrev;
...
//	CPageLangDisplay pageDisplay("Display", getAbrev().GetLength());
...
//	CPageDictAffixes pageAffixDicts("Affix Dicts");
...
CCarlaLanguage::~CCarlaLanguage()
{
	...
	{
			CString s;
			s.Format("Please report this bug: Deleting AmpleDLLWrapper gave error: %s", sError);
			AfxMessageBox(s);
	}
...
void CCarlaLanguage::finishCreate()
{
	...
	{
		CString s(getName());
		s+= "'s count of transfer sequences does not equal its count of transfer models.  Something has become corrupted, and you should get help from carlastudio_support@sil.org";
		AfxMessageBox(s);
	}
	if(m_pTransferSequences.GetSize() != (getProject()->getLangDocs().GetSize() -1))
	{
		CString s(getName());
		s+= "'s count of transfer sequences does not equal number of other languages in the project.  Something has become corrupted, and you should get help from carlastudio_support@sil.org";
		AfxMessageBox(s);
	}
	...
	for(int i=m_pTransferSequences.GetSize()-1; i>=0; i--)
	{
		...
		catch(CString sError)
		{
			...
			CString s("The Transfer Sequence for (src) language ");
			s+= getName();
...
/*	if((m_mfs.m_sCtrlFilesDirectory.GetLength() !=0 )
		...
		AfxMessageBox("Warning: saving a language to a new location has not been tested (or implemented properly).");
*/
	m_mfs.m_sCtrlFilesDirectory = getLanguageDirectory(lpszProjectDir);
}
