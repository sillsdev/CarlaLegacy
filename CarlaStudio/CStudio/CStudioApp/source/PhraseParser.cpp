...
*/ 		}
	...
	catch (CProcessFailure failure)
	{
		...
		sResult.Format("<error>%s<\\error>", str);
		return sResult;
	}
	catch (CString sError)
	{
		sResult.Format("<error>%s<\\error>", sError);
		return sResult;
	}
	catch (LPCTSTR lpzError)
	{
		sResult.Format("<error>%s<\\error>", lpzError);
		return sResult;
	}
	return sResult;
}
...
*/ 		}
	...
	// DO THE PARSING
		...
		m_pAmpleProcess->setOutputStyle("AResult");
...
void CPhraseParser::prepareAmpleProcess()
{
	...
	//LOG2("CPhraseParser creating ample process", "\r\n");
...
BOOL CPhraseParser::preprocessDicts(CProcessStatus* pStatus)
{
	...
	if(!bOK)
		throw CString("There was a problem refreshing the dictionaries.  You'll need to open a source document and parse it to see what the problem is.");
