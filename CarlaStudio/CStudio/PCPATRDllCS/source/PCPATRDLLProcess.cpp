...
void CPCPATRDllProcess::writeToStream(ostream& fout) const
{
  ...
  fout << "\\SentenceFinalPunctuation " << m_sSentenceFinalPunctuation << "\n";
...
void CPCPATRDllProcess::processANAFile(CProcessStatus& status)
{
  ...
  try
...
#ifndef hab245
	  CString sBase;
	  sBase.Format("%s%d", getTempFileNameBase(status), status.m_iProcNumber);
#else // hab245
	...
	m_iRootGlossSetting--;	// adjust the value back
	  ...
	  checkResults(hr, "disambiguate ANA file", pPatr);
	  hr = pPatr->CloseLog();
	  checkResults(hr, "close log file", pPatr);
#else // hab245
	  checkResults(hr, "coinitialize");
	  hr = CLSIDFromProgID(L"SIL.CARLA.PatrParser.1", &clsid);
  ...
  if (FAILED(hr))
	{
	  ...
	  CString s = "PatrProcess DLL process failed to " + s2;
	  HRESULT hr2;
