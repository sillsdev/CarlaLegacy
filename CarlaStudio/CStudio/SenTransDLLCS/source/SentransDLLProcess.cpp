...
void CSenTransDllProcess::readParametersFromSFMFile(SFMFile *f)
{
	...
	while(f->getField(sMarker, sField))
	{
		...
		else f->throwParseFailure("SENTRANSDLLProcess", sMarker, sField);
	}
}
...
void CSenTransDllProcess::processANAFile(CProcessStatus& status)
{
  ...
  switch(m_iFunction)
	...
	return;
	  m_sRuleFile = pSourceMFS->getShortCtrlFilePath("disam.sen");
	  break;
	case kAdjustSrcGlossing:
	  if(status.getInputLang()->getSentransGlossAdjustmentModel().m_rules.getSize() ==0)
	return;
	  m_sRuleFile = pSourceMFS->getShortCtrlFilePath("adj.sen");;
	  break;
	...
	return;
	  m_sRuleFile = pSourceMFS->getShortCtrlFilePath("ana.sen");;
	  break;
  ...
  try
	...
	throw(CProcessFailure(this,
	  ...
	  if (FAILED(hr))
	throw(CProcessFailure(this,
			 "SenTransCOM DLL process failed to apply rules to file"));
	  pSentrans->Release();
	  pSentrans = NULL;
	  CoUninitialize();
...
LPCTSTR  CSenTransDllProcess::getDisplayName() const
{
	switch(m_iFunction)
	{
