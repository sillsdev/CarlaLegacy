/* ToneGen.C - Perform STAMP synthesis and generate tone-marked words
 ***************************************************************************
 *
 ...
 */
while ((k = getopt(argc, argv, "ac:d:D:f:i:mno:qrtTuv/z:Z:")) != EOF)
	{
	switch (k)
		{
...
if (sStamp_m.pszSegFileName == (char *)NULL)
	{
	  fprintf(stderr,
		"ToneGen: Segment Inventory file not specified in control file\n");
	  exitSafely(2);
	}
  else if (load_segs(sStamp_m.pszSegFileName, bVerify_m, &sStamp_m) > 0)
	{
	  fprintf(stderr, "ToneGen: Cannot load segment file %s\n",
...
do  {
	...
	fclose(infp);
	fflush(outfp);
	checkFileError( outfp, "TONEGEN", outfilename );
	fclose(outfp);
 ...
 */
fprintf(sStamp_m.pLogFP,
	"\nTONEGEN STATISTICS: %4ld WORDS processed.\n",
	uiWordCount_m);
/*
