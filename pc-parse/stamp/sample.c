/* SAMPLE.C - Synthesis from AMPLE output
 ***************************************************************************
 *
...
static void	outstats(void);
static void	tsproc(void);
 ...
 */
while ((k = getopt(argc, argv, "ac:d:f:i:mno:qrtuv/z:Z:")) != EOF)
	{
	switch (k)
		{
...
{
fprintf(pOutFP_in,
	"SAMPLE: Synthesis(AMPLE(text)) = synthesized text\n");
...
#endif
fprintf( pOutFP_in, "       Synthesis Performed %s", pszTime_in );
}
 ...
 */
getAmpleCmd("Transfer file (xxzzTR.CHG) [ignored]: ", infname, BUFSIZE);
/*
...
do  {
	...
	/*
	 *  Initialize for the word processing loop (get the first word)
	 */
	...
	if (    (pWord != NULL) &&
		...
		{
		sStamp_m.uiAmbiguityCount = getStringListSize(pWord->pNewWords);
	...
	fclose(infp);
	fflush(outfp);
	checkFileError( outfp, "SAMPLE", outfilename );
	fclose(outfp);
...
if (sStamp_m.uiCalledMEC || sStamp_m.uiCalledSEC ||
	...
	if (sStamp_m.uiCalledPEC)	/* 2.1b1 hab */
		fprintf(sStamp_m.pLogFP,
	   "     Punctuation environment constraints failed %lu times, succeeded %lu.\n",
