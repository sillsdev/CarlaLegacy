/* dtbstringin.c - read the next record from an AMPLE Input ANA string
 ***************************************************************************
 *
 * #include "template.h"
 *
 * WordTemplate * readTemplateFromAnalysisString(FILE * pInputFP_in,
...
static char ** readAnalysisRecordString(const char * pszInputString_in, StampSetup *pSetup_io);
 ...
 */
static char ** readAnalysisRecordString(pszInputString_in, pSetup_io)
...
while ((pszLine = readLineFromString(pszInputString_in, NUL, pSetup_io)) != NULL)
    {
    if (pszLine[0] == NUL)
	{
	/*
	...
	apszRecord[iIndex] = strcat(reallocMemory(apszRecord[iIndex],
				       ...
				       pszLine+1);
	*/
	int cch = (int)strlen(apszRecord[iIndex]) + (int)strlen(pszLine);
 ...
 *    readLineFromString
 * ARGUMENTS
 *    const char * pszInputString_in - input string
 ...
 */
char *readLineFromString(pszInputString_in, cCommentChar_in, pSetup_io)
const char *pszInputString_in;
int	 cCommentChar_in;
StampSetup *pSetup_io;
{
...
if (pSetup_io->bNewTextBuffer) {	//This is a reset for a new text buffer.
	pSetup_io->bNewTextBuffer = FALSE;
  	position = NULL;
}

if (!position) // Return visit?
	position = (char *)pszInputString_in;

begin = position;

if (!(*position))	//End of Input	
 	return NULL; //In case they send us another.

for ( p = position; *p != NUL ; p++); // end of field

if (!strchr(begin, '\\')) // Blank line end of record.
	begin = p;

position = ++p;	//For the next line or EOL

/*
 *  get rid of any comments or trailing whitespace
 */
if (cCommentChar_in && (p = strchr(begin, cCommentChar_in)) != NULL)
    *p = NUL;
...
/*****************************************************************************
 * NAME
 *    readTemplateFromAnalysisString
 * DESCRIPTION
 ...
 */
WordTemplate * readTemplateFromAnalysisString(pszInputString_in, pTextCtl_in, pSetup_io)
const char *pszInputString_in;
const TextControl *pTextCtl_in;
StampSetup *pSetup_io;
{
...
ppszAnalysisRecord = readAnalysisRecordString(pszInputString_in, pSetup_io);
 ...
 * 16-Apr-2013  rke - modified to allow string input instead of file
 */
