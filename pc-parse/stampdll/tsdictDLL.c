...
/*      tsdictdll.c - dictionary functions for transfer and synthesis
 ***************************************************************************
 *
 * int loadStampDictionaryDLL(const char *pszFilename_in,
 *				int		eDicType_in,
 *				StampData *pStamp_io)
 *
 * //StampMorpheme * searchStampDictionaryDLL(unsigned    type,
 *					 char *	     name,
 *					 unsigned    category,
 *					 unsigned    catmatch,
 *					 StampData * pStamp_in)
 *
 * void writeStampDictionaryDLL(FILE *      pOutputFP_in,
 *                           StampData * pStamp_in)
 *
 * //void freeStampDictionary(StampData * pStamp_io)
 *
 * Wraps so can access static functions
 * void add_affixwrap(char *recp, int iType, StampData *pStamp_in)
 * void add_rootwrap(char *recp, StampData *pStamp_in)
 * void addStampDictEntrywrap(char *pRecord_in, StampData *pStamp_in)
...
static void  insert_dict P((StampMorpheme * new,
			    StampData *     pStamp_in));
static int  check_dictDLL(StampMorpheme *new, StampData *pStamp_in);
void freeStampMorphemeDLL(StampMorpheme * mp);
...
// Wrap the statics so we can call them.
...
void add_affixwrap(char *recp, int iType, StampData *pStamp_in)
{
	...
	break;
    default:
	pszDictErrorHead_m = "Error in add_affixwrap: ";
	break;
    }
	add_affix(recp, iType, pStamp_in);
	return;
}
void add_rootwrap(char *recp, StampData *pStamp_in)
{
	pszDictErrorHead_m = "ROOT DICTIONARY: ";
	add_root(recp, pStamp_in);
	return;
}
...
void addStampDictEntrywrap(char *pRecord_in, StampData *pStamp_in)
{
	...
	addStampDictEntry(pRecord_in, pStamp_in);
	return;
}
...
for (   x = (int)strlen((char *)xcats), p = line + strspn(line, szWhitespace_m);
        (*p != '\0') && (x < BUFSIZE) ;
        p = line, ++x )
    {
    line = isolateWord(p);
...
char *		nextp;
char *		endp;
char *      pszMorph;			//For allomorphID
StampMorpheme	morpheme;
StampAllomorphList * ap;		/* current allomorph being built */
StampAllomorphList * alp;		/* for allomrophID */
...
StampAllomorphList * elsewhere_prec;	/* ptr to allomorph preceding the
					 *    elsewhere allomorph */
int i, iAlloCount;	//for allomorphID
...
if (morpheme.pszMorphname  && morpheme.pAllomorphs && !bad_allo)
    {
    for (iAlloCount = 0, ap = morpheme.pAllomorphs ; ap ; ap = ap->pNext, ++iAlloCount)
     ...
     *  this is done here so that we can number the ids
	 */
	if (pStamp_in->bEnableAllomorphIDs)
		{
			pszMorph = morpheme.pszMorphname;
			if (pszMorph == NULL)
			for (i = iAlloCount, alp = morpheme.pAllomorphs ; alp ; alp = alp->pNext, --i )
				{
					if (alp->a.pszAllomorphID == NULL)
						alp->a.pszAllomorphID = build_allomorph_id(pszMorph, i);
				}
		}
...
char *		endp;
char *		mname;
char *		pszMorph = NULL;	//allomorphID
StampMorpheme	morpheme;
StampAllomorphList * ap;
StampAllomorphList * alp;		/* for allomorphID */
StampAllomorphList * elsewhere_allo;	/* ptr to the elsewhere allomorph  */
StampAllomorphList * elsewhere_prec;	/* ptr to allomorph preceding the
					 *   elsewhere allomorph */
int		is_elsewhere_allo;	/* flag if current allomorph is
...
int		iAlloCount;	/* for allomorphID */
PropertySet_t	mprops;			/* temp for morpheme properties */
/*
 *  initialize the morpheme structure
 */
...
for ( rp = recp ; *rp != NUL ; rp = endp )
    ...
    switch (code)
	    ...
	    rp       = nextp;
            ...
            rp = nextp;	// is this and the one above necessary?
            break;
	...
	if (pStamp_in->bEnableAllomorphIDs)
		{
			...
			if (pszMorph == NULL)
				pszMorph = "";
			for (k = iAlloCount, alp = morpheme.pAllomorphs ; alp ; alp = alp->pNext, --k )
				{
					if (alp->a.pszAllomorphID == NULL)
						alp->a.pszAllomorphID = build_allomorph_id(pszMorph, k);
				}
		}
 ...
 *    none
 */
int loadStampDictionaryDLL(pszFilename_in, eDicType_in, pStamp_io)
...
for ( pAlloL = pList_io ; pAlloL ; pAlloL = pAlloNext )
	...
	if (pAlloL->a.pszAllomorphID)
		freeMemory(pAlloL->a.pszAllomorphID);
 ...
 */
if (check_dictDLL(new,	pStamp_io)) {
    if (pStamp_io->pLogFP != NULL)
	fprintf(pStamp_io->pLogFP,
		"%smorphname already in dictionary replaced: %s\n",
 ...
 * NAME
 *    check_dictDLL
 *    (modified from check_dict() to overwrite old entry with new duplicate 15-Nov-2016 RE)
 * ARGUMENTS
 *    mp - pointer to the new morpheme structure
 *    pStamp_in - Dictionary and other data
 * DESCRIPTION
 ...
 */
static int check_dictDLL(new, pStamp_in)
StampMorpheme *new;
StampData *	pStamp_in;
{
StampMorphemeList *mp;

for (	mp = (StampMorphemeList *)findDataInTrie(pStamp_in->pDictionary,new->pszMorphname) ;
	mp ;
	mp = mp->mlink )
    {
    if (!strcmp(new->pszMorphname,mp->m.pszMorphname))
        {
        if (new->iMorphType & mp->m.iMorphType & ROOT)
            {                           /* root category must match also */
            if (strpbrk( (char *)mp->m.u.pRootCategories, (char *)new->u.pRootCategories) == NULL)
                continue;               /* different type of root */
            }
		freeStampMorphemeDLL(&mp->m);	// Remove old morpheme structure values
		/*
		 *  copy the values
		 */
		mp->m.pszMorphname = new->pszMorphname;
...
//			mp->m.u.pInfixEnv = new->u.pInfixEnv;           /* infix environment */
        ...
        return( 1 );            /* found and replaced it! */
        }
    }
...
/*************************************************************************
 * NAME
 *    freeStampMorphemeDLL
 *	  for inserting a new morpheme and not deleting the position in the morpheme list (Nov 15, 2016 RE)
 * ARGUMENTS
...
// The below cleanup must come at the end of execution.
// u.pRootCategories is a linked list. All dictionary root entries point to different
// nodes correspoinding to their part of speech.
...
freeStampAllomorphListDLL( mp->pAllomorphs );            /* it checks for NULL */
// freeMemory( (void *)mp );
}

/*************************************************************************
 * NAME
 *    freeStampAllomorphListDLL
 * ARGUMENTS
 ...
 *    none
 */
void freeStampAllomorphListDLL(ap)
StampAllomorphList * ap;
{
...
// This are kept as a unique list in the main structure. pAlloEnvList
// These should not be cleared at this time.
//	if (ap->a.pAlloEnvironment != NULL)
//		{
 ...
 *    removeStampMorpheme
 * DESCRIPTION
 *    function to remove a morpheme from StampMorphemeList
 * RETURN VALUE
 *    pointer to the modified list
 */
static VOIDP removeStampMorpheme(pOld_in, pList_io)
VOIDP	pOld_in;
VOIDP	pList_io;
{
StampMorphemeList *	mp;
StampMorphemeList *	mpPrevious;

mpPrevious = NULL;
for ( mp = (StampMorphemeList *)pList_io ; mp ; mp = mp->mlink )
    {
    if (mp == (StampMorphemeList *)pOld_in)
	{
	if (pOld_in == pList_io)
	    pList_io = (VOIDP)(mp->mlink);
	else
	    mpPrevious->mlink = mp->mlink;
	/*
	 *  free the memory used for this allomorph
	 */
	freeStampMorpheme(&mp->m);
	break;
	}
    mpPrevious = mp;
    }
...
/*************************************************************************
 * NAME
 *    writeStampDictionaryDLL
 * DESCRIPTION
 *    Display the contents of the entire dictionary (gulp).
 * RETURN VALUE
 *    none
 */
void writeStampDictionaryDLL(pOutputFP_in, pStamp_in)
FILE *		pOutputFP_in;
StampData *	pStamp_in;
{
