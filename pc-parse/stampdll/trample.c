/* TRAMPLE.C - TRansfer of AMPLE output
 ***************************************************************************
 *
 ...
 * OUTPUT is written to a similar file.
 *
 ***************************************************************************
 ...
 */
while ((k = getopt(argc, argv, "c:d:f:i:mno:qrtuv/z:Z:")) != EOF)
	{
	switch (k)
		{
...
", stderr); fputs("\
-t       Trace transfer (produces voluminous output)\n\
...
{
fprintf(pOutFP_in,
	"TRAMPLE: TRansfer(AMPLE(text)) = transferred analysis\n");
...
#endif
fprintf( pOutFP_in, "       Morphological Transfer Performed %s", pszTime_in );
}
 ...
 */
getAmpleCmd("Synthesis file (zzSYNT.CHG) [ignored]: ", infname, BUFSIZE);
/*
...
do  {
	...
	writeStampWord(sWords_m.pCurrentWord, outfp, outfilename, &sStamp_m);
	if (bMonitorProgress_m)
		{
	...
	fclose(infp);
	fflush(outfp);
	checkFileError( outfp, "TRAMPLE", outfilename );
	fclose(outfp);
